...
export const DEFAULT_ROUTER_DEFAULT_SKILL = 'friend_im'
...
export function createAgentRouter({
  ...
  const scheduler = createMailboxScheduler({
    ...
    async handleItem(item, { mailboxKey, conversationKey }) {
      ...
      const canFallbackToDefault = false
      ...
      if (execution?.ownerReport != null && typeof notifyOwner === 'function') {
        ...
        await notifyOwner({
          ...
          // AgentSquared friend conversations are owner-visible by default.
          // Conversation control only decides whether the peer session continues.
        })
      }
