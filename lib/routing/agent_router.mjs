import { findLocalOfficialSkill } from '../conversation/local_skills.mjs'
import { normalizeConversationControl, normalizeSharedSkillName, resolveInboundConversationIdentity } from '../conversation/policy.mjs'
import { buildReceiverBaseReport } from '../conversation/templates.mjs'

function clean(value) {
  return `${value ?? ''}`.trim()
}

function describeRouterError(error = null, seen = new Set()) {
  if (error == null) {
    return ''
  }
  ...
  const cause = describeRouterError(error.cause, seen)
  if (cause) {
...
function runtimeRejectFromError(error = null) {
  return {
    code: Number.parseInt(`${error?.code ?? 500}`, 10) || 500,
    message: clean(error?.message) || 'local runtime execution failed',
    stage: clean(error?.a2OpenClawStage || error?.a2RuntimeStage || error?.stage),
    runtimeAdapter: clean(error?.runtimeAdapter || error?.a2RuntimeAdapter),
    failureKind: clean(error?.a2FailureKind || error?.failureKind || error?.kind),
    detail: describeRouterError(error),
    stack: clean(error?.stack),
    noFallback: Boolean(error?.a2NoFallback || error?.runtimeAdapter || error?.a2RuntimeAdapter)
  }
}
...
function withLocalSkillMetadata(item = null, localSkill = null) {
  ...
  return {
    ...
    request: {
      ...
      params: {
        ...
        metadata: {
          ...metadata,
          localSkillMaxTurns: localSkill?.maxTurns ?? 1
        }
      }
    }
  }
...
function synthesizeRuntimeUnavailableExecution({
  ...
  const finalSkill = normalizeSharedSkillName(defaultSkill || selectedSkill)
  ...
  return {
    ...
    ownerReport: {
      title: '**🅰️✌️ AgentSquared message from remote agent failed**',
      ...
      message: [
        ...
        `**Sender:** \`${remoteAgentId || 'unknown'}\` → **Recipient:** \`local agent\``,
        `**Status:** \`system-error\` | **Total turns:** \`${conversation.turnIndex}\``,
        '**Time:** unknown → unknown',
        `**Skill:** sender:\`unknown\` → recipient:\`${finalSkill || '(none)'}\``,
        '',
        '**What happened**',
...
function synthesizeSkillUnavailableExecution({
  item,
  selectedSkill,
  mailboxKey,
  localAgentId = ''
  ...
  const metadata = extractConversationMetadata(item)
  const remoteAgentId = clean(item?.remoteAgentId) || clean(metadata.from) || 'the remote agent'
  const conversation = normalizeConversationControl(metadata, {
    defaultTurnIndex: Number.parseInt(`${metadata.turnIndex ?? 1}`, 10) || 1,
    defaultDecision: 'done',
    defaultStopReason: 'skill-unavailable'
  })
  const conversationKey = clean(metadata.conversationKey)
  const requestedSkill = normalizeSharedSkillName(selectedSkill || item?.suggestedSkill || item?.defaultSkill)
  const peerReplyText = `I cannot use the requested AgentSquared skill${requestedSkill ? ` "${requestedSkill}"` : ''} because it is not available in my local official A2 Skills checkout. Please install or update the official AgentSquared Skills, or submit this workflow to A2 official first.`
  const finishedAt = nowISO()
  const ownerReport = buildReceiverBaseReport({
    localAgentId,
    remoteAgentId,
    incomingSkillHint: requestedSkill,
    selectedSkill: requestedSkill,
    conversationKey,
    receivedAt: clean(metadata.sentAt) || finishedAt,
    inboundText: extractInboundText(item),
    peerReplyText,
    repliedAt: finishedAt,
    skillSummary: `Rejected the AgentSquared request because the requested skill${requestedSkill ? ` "${requestedSkill}"` : ''} is not installed locally as an official A2 skill.`,
    conversationTurns: conversation.turnIndex,
    stopReason: 'skill-unavailable',
    conversationTurnDetails: [],
    detailsAvailableInInbox: true,
    remoteSentAt: clean(metadata.sentAt),
...
export const DEFAULT_ROUTER_DEFAULT_SKILL = ''
export const DEFAULT_ROUTER_SKILLS = []

export function normalizeRouterSkills(skills = DEFAULT_ROUTER_SKILLS) {
  const seen = new Set()
  const out = []
  for (const value of skills) {
    const skill = normalizeSharedSkillName(value)
...
export function chooseInboundSkill(item, {
  ...
  const suggestedSkill = normalizeSharedSkillName(item?.suggestedSkill)
  const requestDefaultSkill = normalizeSharedSkillName(item?.defaultSkill)
  const localDefaultSkill = normalizeSharedSkillName(defaultSkill)
  return suggestedSkill || requestDefaultSkill || localDefaultSkill || ''
}
...
export function createAgentRouter({
  ...
  notifyOwner = null,
  localAgentId = '',
  resolveLocalSkill = findLocalOfficialSkill,
  onStreamEvent = null,
  onRespond,
  onReject
} = {}) {
  ...
  const normalizedRouterSkills = normalizeRouterSkills([])
  const normalizedDefaultSkill = normalizeSharedSkillName(defaultSkill)

  function shouldFallbackToDefaultSkill(reject = null) {
    if (reject?.noFallback || clean(reject?.runtimeAdapter) || clean(reject?.stage)) {
      return false
    }
  ...
  const scheduler = createMailboxScheduler({
    ...
    async handleItem(item, { mailboxKey, conversationKey }) {
      ...
      const localSkill = typeof resolveLocalSkill === 'function'
        ? resolveLocalSkill(selectedSkill)
        : { available: false }
      const executionItem = withLocalSkillMetadata(item, localSkill)

      let execution
      if (!localSkill?.available) {
        execution = synthesizeSkillUnavailableExecution({
          item: executionItem,
          selectedSkill,
          mailboxKey,
          localAgentId
        ...
        try {
          execution = await executeInbound({
            item: executionItem,
            selectedSkill: localSkill.name || selectedSkill,
            mailboxKey,
            emitStreamEvent: typeof onStreamEvent === 'function'
              ? (event) => onStreamEvent(executionItem, event)
              : null
          })
        } catch (error) {
          execution = {
            reject: runtimeRejectFromError(error)
          }
        }
      }
      ...
      const shouldNotifyOwner = Boolean(
        execution?.ownerReport != null
        && typeof notifyOwner === 'function'
        && conversation.final
      )
      if (shouldNotifyOwner) {
        await notifyOwner({
          ...
          // Owner notifications are emitted only after a conversation reaches a
          // final state. Intermediate turns stay on the peer stream so the
          // initiating agent can complete the A2A workflow before the owner gets
          // the final report.
        })
      }
      ...
      return {
        selectedSkill,
        rejected: false,
        ownerReportDelivered: shouldNotifyOwner,
        releaseConversationLock: Boolean(conversation.final || conversation.decision === 'done'),
