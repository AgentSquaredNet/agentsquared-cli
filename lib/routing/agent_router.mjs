...
function synthesizeRuntimeUnavailableExecution({
  ...
  return {
    ...
    ownerReport: {
      ...
      message: [
        '**Conversation result**',
        `- Sender: ${remoteAgentId}`,
        '- Recipient: local agent',
        ...(conversationKey ? [`- Conversation ID: ${conversationKey}`] : []),
        `- Total turns: ${conversation.turnIndex}`,
        '- Sender skill: unknown',
        `- Receiver skill: ${finalSkill}`,
        '- Conversation status: system-error',
        '',
        '**What happened**',
