...
export function buildStandardRuntimeOwnerLines(report = {}) {
  ...
  const gatewayHealthy = Boolean(gatewayStatus.started && clean(gatewayStatus.gatewayBase))
  const hostHealthy = Boolean(gatewayStatus.hostRuntime?.ok)
  const relayHealthy = Boolean(gatewayStatus.relay?.ok)
  const lines = [
    'Runtime Status:',
    `A2 gateway: ${gatewayHealthy ? 'healthy' : 'not healthy'}${clean(gatewayStatus.gatewayBase) ? ` at ${clean(gatewayStatus.gatewayBase)}` : ''}${clean(gatewayStatus.peerId) ? `, peer ${clean(gatewayStatus.peerId)}` : ''}.`,
    `Host runtime adapter: ${hostHealthy ? 'healthy' : 'not healthy'} (${clean(overall.hostRuntime) || 'none'}; ${clean(gatewayStatus.hostRuntime?.detail) || 'no detail'}).`,
    `Official AgentSquared Relay: ${relayHealthy ? 'healthy' : 'not healthy'} (${clean(overall.relayApiBase) || 'unknown'}; ${clean(gatewayStatus.relay?.detail) || 'no detail'}).`,
    `Local runtime: @agentsquared/cli ${clean(overall.runtimePackageVersion) || 'unknown'}, revision ${clean(overall.runtimeRevision) || 'unknown'}, update status ${latestStatusLabel(overall.latestStatus?.status)}.`
  ]

  if (runtimeUpdate.initialInstall) {
    lines.push('Runtime update: this is the first recorded runtime report for this local AgentSquared setup.')
