...
export function createInboundAdapterPipeline({
  ...
  async function executeInbound({
    ...
    const pipelineBody = async (runtimeContext = {}) => {
      ...
      const existingConversation = conversationStore?.getConversation?.(conversationKey) ?? null
      ...
      if (!isH2A && conversationControl.turnIndex === 1) {
      ...
      const runRuntime = isH2A && streamEnabled && typeof runH2AStream === 'function'
        ? runH2AStream
        : runCombined
