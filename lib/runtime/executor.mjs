...
function normalizeOwnerDeliveryResult(result = null) {
  ...
  return {
    ...
    delivered,
    status: delivered ? 'sent' : timedOut ? 'unknown_maybe_sent' : 'failed',
...
export function createOwnerNotifier({
  ...
  async function notifyViaHost(context) {
    ...
    const duplicateFinal = shouldDeliverToOwner && finalConversationKey
    ...
    let acceptedDelivery = {
      attempted: false,
      delivered: false,
      status: shouldDeliverToOwner ? 'pending' : 'audit-only',
      mode: normalizedHostRuntime || 'host',
      reason: shouldDeliverToOwner ? 'owner-notification-accepted' : 'conversation-intermediate',
      acceptedAt: nowISO()
    }
    let shouldScheduleHostDelivery = Boolean(shouldDeliverToOwner && hostAdapter)
    if (duplicateFinal) {
      acceptedDelivery = {
        delivered: false,
        attempted: false,
    ...
    if (finalConversationKey && context?.ownerReport?.finalize && shouldScheduleHostDelivery) {
    ...
    if (shouldScheduleHostDelivery) {
      ...
      setTimeout(async () => {
        ...
        if (finalConversationKey && Boolean(normalized.delivered) && context?.ownerReport?.finalize) {
    ...
    } else if (finalConversationKey && context?.ownerReport?.finalize) {
      pendingFinalConversationKeys.delete(finalConversationKey)
      if (acceptedDelivery.status === 'suppressed_duplicate') {
        deliveredFinalConversationKeys.add(finalConversationKey)
      }
    }
    ...
    return {
      delivered: true,
      mode: normalizedHostRuntime || 'host',
      notificationAccepted: shouldDeliverToOwner && acceptedDelivery.status !== 'suppressed_duplicate',
      notificationStatus: 'handled-by-agentsquared',
      entryId: value.entry.id,
      totalCount: value.index.totalCount,
      ...
      ownerDelivery: {
        ...
        mode: normalizedHostRuntime || 'host',
        reason: acceptedDelivery.reason
      }
    }
  }
