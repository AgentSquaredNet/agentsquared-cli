...
const STOP_REASON_ALIASES = new Map([
  ['goal-satisfied', 'completed'],
  ['no-new-information', 'completed'],
  ['max-turns-reached', 'completed'],
  ['peer-requested-stop', 'completed'],
  ['single-turn', 'completed'],
  ['owner-approval-required', 'safety-block'],
  ['unsafe-or-sensitive', 'safety-block'],
  ['receiver-budget-limit', 'system-error'],
  ['timeout', 'system-error'],
  ['receiver-runtime-unavailable', 'system-error']
])
...
export function normalizeConversationControl(raw = {}, {
  ...
  const stopReason = VALID_STOP_REASONS.has(normalizedStopReason)
    ...
    : VALID_STOP_REASONS.has(STOP_REASON_ALIASES.get(normalizedStopReason))
      ? STOP_REASON_ALIASES.get(normalizedStopReason)
      : VALID_STOP_REASONS.has(normalizedDefaultStopReason)
        ? normalizedDefaultStopReason
        : VALID_STOP_REASONS.has(STOP_REASON_ALIASES.get(normalizedDefaultStopReason))
          ? STOP_REASON_ALIASES.get(normalizedDefaultStopReason)
          : ''
