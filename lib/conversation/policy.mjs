...
const SKILL_ALIASES = new Map([
  ['friend-im', 'friend_im'],
  ['friendim', 'friend_im'],
  ['peer-session', 'friend_im'],
  ['peer_session', 'friend_im'],
  ['mutual-learning', 'agent_mutual_learning'],
  ['mutual_learning', 'agent_mutual_learning'],
  ['agent-mutual-learning', 'agent_mutual_learning']
])
...
export function normalizeSharedSkillName(value = '', fallback = '') {
  ...
  if (normalizedValue) {
    return SKILL_ALIASES.get(normalizedValue) || normalizedValue
  }
  const normalizedFallback = clean(fallback).toLowerCase()
  return SKILL_ALIASES.get(normalizedFallback) || normalizedFallback
}
