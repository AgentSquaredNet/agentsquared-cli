...
export function resolveConversationMaxTurns({
  conversationPolicy = null,
  sharedSkill = null,
  fallback = 1
} = {}) {
  const boundedFallback = clampConversationMaxTurns(fallback, 1)
  const senderMaxTurns = conversationPolicy && typeof conversationPolicy === 'object'
    ? conversationPolicy.maxTurns
    : null
  const sharedMaxTurns = sharedSkill && typeof sharedSkill === 'object'
    ? sharedSkill.maxTurns
    : null
  if (!isValidConversationMaxTurns(senderMaxTurns) || !isValidConversationMaxTurns(sharedMaxTurns)) {
    return boundedFallback
  }
  const normalizedSenderMaxTurns = Number.parseInt(`${senderMaxTurns}`, 10)
  const normalizedSharedMaxTurns = Number.parseInt(`${sharedMaxTurns}`, 10)
  return normalizedSenderMaxTurns === normalizedSharedMaxTurns
    ? normalizedSenderMaxTurns
    : boundedFallback
}
