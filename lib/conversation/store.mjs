...
function formatTranscript(turns = [], {
  maxTurns = Number.parseInt(process.env.A2_CONVERSATION_TRANSCRIPT_TURNS ?? '6', 10) || 6,
  maxTextLength = Number.parseInt(process.env.A2_CONVERSATION_TRANSCRIPT_TEXT_CHARS ?? '260', 10) || 260,
  maxChars = Number.parseInt(process.env.A2_CONVERSATION_TRANSCRIPT_MAX_CHARS ?? '3600', 10) || 3600
} = {}) {
  if (!Array.isArray(turns) || turns.length === 0) {
    return ''
  }
  const boundedMaxTurns = Math.max(2, maxTurns)
  const normalizedTurns = turns.length <= boundedMaxTurns
    ? turns
    : [
        turns[0],
        { omitted: turns.length - boundedMaxTurns, turnIndex: '...' },
        ...turns.slice(-(boundedMaxTurns - 1))
      ]
  const transcript = normalizedTurns.map((turn) => {
    if (turn?.omitted) {
      return `... ${turn.omitted} earlier turn(s) omitted from this prompt transcript for latency.`
    }
    const lines = [
      `Turn ${turn.turnIndex}:`,
      `- Remote message: ${excerpt(turn.inboundText, maxTextLength) || '(empty)'}`,
      `- My reply: ${excerpt(turn.replyText, maxTextLength) || '(empty)'}`,
  ...
  return transcript.length > maxChars
    ? `${transcript.slice(0, Math.max(0, maxChars - 3))}...`
    : transcript
}
...
export function createLiveConversationStore() {
  ...
  function appendTurn({
    ...
    const normalized = {
      ...
      selectedSkill: clean(selectedSkill) || clean(current.selectedSkill),
      updatedAt: nowISO(),
      turns: [
  ...
  function closeConversation(conversationKey, summary = '') {
