...
export function platformChannelLabel(channelKind = '') {
  const normalized = clean(channelKind).toLowerCase()
  if (normalized === 'h2a') {
    return 'H2A'
  }
  if (normalized === 'api') {
...
export function buildInboundPlatformContext({
  ...
  const rawChannelKind = clean(metadata.channelKind).toLowerCase()
  const channelKind = rawChannelKind === 'h2a' || rawChannelKind === 'api' ? rawChannelKind : 'a2a'
  const fromHuman = clean(metadata.fromHuman)
  const fromAgent = channelKind === 'h2a' || channelKind === 'api' ? '' : clean(metadata.from || remoteAgentId)
  const toAgent = clean(metadata.toAgent || metadata.to || localAgentId)
  return buildPlatformContextBlock({
    channelKind,
    senderAgent: fromAgent,
    senderSurface: channelKind === 'api' ? 'OpenAI-compatible API' : channelKind === 'h2a' ? 'browser human chat' : '',
    senderHuman: fromHuman || (channelKind === 'h2a' || channelKind === 'api' ? clean(metadata.from).replace(/^human:/, '') : humanFromAgentId(fromAgent)),
    ...
    transport: channelKind === 'api'
      ? 'AgentSquared API relay'
      : channelKind === 'h2a'
        ? clean(metadata.browserBridge) || 'AgentSquared H2A bridge'
        : 'AgentSquared P2P',
