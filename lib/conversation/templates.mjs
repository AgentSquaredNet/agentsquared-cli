...
export function buildConversationSummaryPrompt({
  ...
  return [
    ...
    zh
      ? '请用中文输出。只输出一段非常精简的总结，最多 80 个汉字，抓住对话的核心结论和可行动收获。'
      : 'Return one concise paragraph, at most 45 words, capturing the core outcome and actionable takeaways.',
    '',
    ...
    ...(normalizedTurns.length > 0
      ? normalizedTurns.map((turn) => [
          `Turn ${turn.turnIndex}:`,
          `Local/initiator message: ${excerpt(turn.outboundText || turn.inboundText, 800) || '(empty)'}`,
          `Peer reply: ${excerpt(turn.replyText || turn.inboundText, 800) || '(empty)'}`
        ].join('\n'))
      : ['(no turns recorded)'])
  ].join('\n')
}
