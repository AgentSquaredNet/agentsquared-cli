...
export {
  buildSkillOutboundText,
  parseAgentSquaredOutboundEnvelope
} from './platform_context.mjs'
...
function looseStructuredReplyField(text = '', fieldName = '') {
...
function structuredReplyDisplayText(text = '') {
  const raw = clean(text)
  if (!raw || !raw.startsWith('{') || !raw.includes('"peerResponse"')) {
    return raw
  }
  try {
    const parsed = JSON.parse(raw)
    if (parsed && typeof parsed === 'object') {
      return clean(parsed.peerResponse || parsed.peerResponseText || parsed.reply) || raw
    }
  } catch {
    // Some host runtimes may return a malformed A2 envelope with unescaped
    // quotes inside peerResponse. Show the intended peer reply when that
    // known field can still be recovered.
  }
  return looseStructuredReplyField(raw, 'peerResponse') || raw
}
...
export function renderConversationDetails(entry = null, {
  ...
  if (turns.length === 0) {
    ...
    for (const turn of turns) {
      ...
      lines.push(quote(structuredReplyDisplayText(turn.replyText) || '(empty)'))
