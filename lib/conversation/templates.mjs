...
const SUMMARY_WEIGHTED_CHAR_LIMIT = 280

function weightedCharLength(text = '') {
  return [...`${text ?? ''}`].reduce((total, char) => {
    ...
    return total + (latinWeighted ? 1 : 2)
  }, 0)
}

function clampWeightedText(text = '', maxWeightedLength = SUMMARY_WEIGHTED_CHAR_LIMIT) {
  const compact = clean(text).replace(/\s+/g, ' ').trim()
  if (!compact || weightedCharLength(compact) <= maxWeightedLength) {
    return compact
  }
  const suffix = '...'
  const suffixLength = weightedCharLength(suffix)
  let total = 0
  let output = ''
  for (const char of compact) {
    const nextLength = weightedCharLength(char)
    if (total + nextLength + suffixLength > maxWeightedLength) {
      break
    }
    output += char
    total += nextLength
...
export function normalizeConversationSummary(text = '', {
  fallback = '',
  maxWeightedLength = SUMMARY_WEIGHTED_CHAR_LIMIT
} = {}) {
  return clampWeightedText(clean(text) || clean(fallback), maxWeightedLength)
}
