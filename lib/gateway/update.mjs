...
async function runCommand(id, label, bin, args = [], {
  ...
  try {
    ...
    return {
      ...
      hint: clean(error?.killed || error?.signal === 'SIGTERM' || error?.code === 'ETIMEDOUT' ? timeoutHint : '')
    }
  }
}
...
export async function runAgentSquaredUpdate({
  ...
  if (skipCli) {
    ...
    steps.push(await runCommand('cli.update', 'CLI update', npmBin, [
      'install',
      '-g',
      '@agentsquared/cli@latest',
      '--fetch-timeout=60000',
      '--fetch-retries=2',
      '--loglevel=notice'
    ], {
      timeoutMs: Number.parseInt(`${args['npm-timeout-ms'] ?? 180000}`, 10) || 180000,
      timeoutHint: 'npm install timed out or stopped producing useful progress. Retry once with `npm install -g @agentsquared/cli@latest --fetch-timeout=60000 --fetch-retries=2 --loglevel=verbose`, or use the existing local a2-cli checkout if you are running an end-to-end test.'
    }))
  }
