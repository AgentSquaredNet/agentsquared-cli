...
function ownerDeliveryStatus(ownerDelivery = null) {
  return clean(ownerDelivery?.status)
    || (ownerDelivery?.delivered ? 'sent' : ownerDelivery?.attempted ? 'failed' : 'audit-only')
}

function isFailedDeliveryStatus(status = '') {
  return ['failed', 'timeout', 'unknown_maybe_sent'].includes(clean(status))
}
...
function deliveryLabel(item) {
  ...
  if (status === 'pending' || status === 'sending' || status === 'accepted') {
    return 'owner-notification-handled'
  }
...
function createSqliteInboxStore({ inboxDir } = {}) {
  ...
  function appendEntry({
    ...
    const file = clean(existingEntry?.file)
      ? path.join(resolvedInboxDir, existingEntry.file)
      : path.join(entriesDir, `${createdAt.replace(/[:.]/g, '-')}_${safeSegment(item?.remoteAgentId ?? ownerReport?.remoteAgentId ?? ownerReport?.targetAgentId)}_${safeSegment(finalId)}.json`)
  ...
  function updateOwnerDelivery(entryId = '', ownerDelivery = null) {
    ...
    jsonWrite(path.join(resolvedInboxDir, entry.file), entry)
    const index = rebuildIndex()
    return { entry, index }
  }
...
function createJsonInboxStore({ inboxDir } = {}) {
  const resolvedInboxDir = path.resolve(clean(inboxDir) || '.')
  const entriesDir = path.join(resolvedInboxDir, 'entries')
  const indexFile = path.join(resolvedInboxDir, 'index.json')
  const inboxMarkdownFile = path.join(resolvedInboxDir, 'inbox.md')
  ...
  function rebuildIndex() {
    ensureDir(entriesDir)
    const files = fs.readdirSync(entriesDir).filter((name) => name.endsWith('.json')).sort()
    const entries = files.map((name) => jsonRead(path.join(entriesDir, name), null)).filter(Boolean)
  ...
  function findEntry(predicate) {
    ensureDir(entriesDir)
    const files = fs.readdirSync(entriesDir).filter((name) => name.endsWith('.json')).sort().reverse()
    for (const name of files) {
      const candidatePath = path.join(entriesDir, name)
      const candidate = jsonRead(candidatePath, null)
      if (candidate && typeof candidate === 'object' && predicate(candidate)) {
        return { entry: candidate, file: candidatePath }
      }
    }
    return null
  }
  ...
  function appendEntry({
    ...
    const conversationKey = extractConversationKey(item, peerResponse, ownerReport)
    const dedupeKey = clean(ownerReport?.dedupeKey ?? item?.dedupeKey)
    const existing = findEntry((candidate) => {
      if (clean(candidate.id) === id) return true
      if (dedupeKey && clean(candidate.ownerReport?.dedupeKey) === dedupeKey) return true
      if (requestId && requestIdFromEntry(candidate) === requestId) return true
      return Boolean(ownerReport?.finalize && conversationKey && clean(candidate.conversationKey) === conversationKey && isFinalOwnerReport(candidate))
    })
    const finalId = clean(existing?.entry?.id) || id
    const file = existing?.file
      || path.join(entriesDir, `${createdAt.replace(/[:.]/g, '-')}_${safeSegment(item?.remoteAgentId ?? ownerReport?.remoteAgentId ?? ownerReport?.targetAgentId)}_${safeSegment(finalId)}.json`)
    const entry = buildEntry({
      resolvedInboxDir,
      existingEntry: existing?.entry ?? null,
      createdAt,
      file,
      agentId,
      selectedSkill,
  ...
  function updateOwnerDelivery(entryId = '', ownerDelivery = null) {
    const existing = findEntry((candidate) => clean(candidate.id) === clean(entryId))
    if (!existing) {
      return null
    }
    const entry = {
      ...
      updatedAt: nowISO(),
      ownerDelivery: {
        ...(existing.entry.ownerDelivery ?? {}),
        ...(ownerDelivery ?? {})
      }
    }
    jsonWrite(existing.file, entry)
    const index = rebuildIndex()
    return { entry, index }
  }

  function findFinalConversationReport(conversationKey = '') {
    return findEntry((entry) => clean(entry.conversationKey) === clean(conversationKey) && isFinalOwnerReport(entry))?.entry ?? null
  }

  function findDeliveredFinalConversationReport(conversationKey = '') {
    return findEntry((entry) => clean(entry.conversationKey) === clean(conversationKey) && isFinalOwnerReport(entry) && entry.ownerDelivery?.delivered)?.entry ?? null
  }
...
export function createInboxStore(options = {}) {
  return DatabaseSync ? createSqliteInboxStore(options) : createJsonInboxStore(options)
}
