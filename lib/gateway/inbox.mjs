...
function hasColumn(db, tableName, columnName) {
  return db.prepare(`PRAGMA table_info(${tableName})`)
    .all()
    .some((column) => clean(column?.name) === clean(columnName))
}
...
function extractConversationKey(item = null, peerResponse = null, ownerReport = null) {
  const metadata = item?.request?.params?.metadata && typeof item.request.params.metadata === 'object'
    ? item.request.params.metadata
    : {}
  return clean(
    peerResponse?.metadata?.conversationKey
      ?? ownerReport?.conversationKey
      ?? ownerReport?.conversationId
...
function deriveRemoteAgentId({ agentId = '', item = null, ownerReport = null } = {}) {
  const localAgentId = clean(agentId)
  const senderAgentId = clean(ownerReport?.senderAgentId)
  const recipientAgentId = clean(ownerReport?.recipientAgentId)
  const candidates = [
    item?.remoteAgentId,
    ownerReport?.remoteAgentId,
    ownerReport?.targetAgentId,
    senderAgentId && senderAgentId !== localAgentId ? senderAgentId : '',
    recipientAgentId && recipientAgentId !== localAgentId ? recipientAgentId : '',
    senderAgentId,
    recipientAgentId
  ]
...
function extractOwnerReportMessageText(ownerReport = null) {
  const turns = Array.isArray(ownerReport?.conversationTurns) ? ownerReport.conversationTurns : []
  const firstTurn = turns[0] ?? null
  return firstClean([
    firstTurn?.outboundText,
    firstTurn?.inboundText,
    ownerReport?.originalText,
    ownerReport?.sentText,
    ownerReport?.inboundText
  ])
}

function extractOwnerReportReplyText(ownerReport = null) {
  const turns = Array.isArray(ownerReport?.conversationTurns) ? ownerReport.conversationTurns : []
  const lastTurn = turns.at(-1) ?? null
  return firstClean([
    lastTurn?.replyText,
    ownerReport?.replyText,
    ownerReport?.peerReplyText
  ])
}

function normalizeOwnerReport(ownerReport = null, {
  conversationKey = '',
  remoteAgentId = '',
  selectedSkill = '',
  peerSessionId = ''
  ...
  if (!ownerReport || typeof ownerReport !== 'object') {
    return ownerReport ?? null
  }
  return {
    ...ownerReport,
    conversationKey: clean(ownerReport.conversationKey) || clean(conversationKey),
    remoteAgentId: clean(ownerReport.remoteAgentId) || clean(remoteAgentId),
    selectedSkill: clean(ownerReport.selectedSkill) || clean(selectedSkill),
    peerSessionId: clean(ownerReport.peerSessionId) || clean(peerSessionId)
  }
}

function extractSelectedSkill(selectedSkill = '', ownerReport = null) {
  return firstClean([
    selectedSkill,
...
function extractPeerSessionId(item = null, ownerReport = null) {
  return firstClean([
    item?.peerSessionId,
    ownerReport?.peerSessionId
  ])
}

function extractRequestId(item = null, ownerReport = null) {
  return firstClean([
    item?.request?.id,
    ownerReport?.requestId
  ])
}

function extractDedupeKey(item = null, ownerReport = null) {
  return firstClean([
    ownerReport?.dedupeKey,
    item?.dedupeKey
  ])
}

function mergeConversationMetadataIntoItem(item = null, conversationKey = '') {
  if (!clean(conversationKey)) {
    return item
  }
  return {
    ...(item ?? {}),
    conversationKey: clean(item?.conversationKey) || clean(conversationKey)
  }
}
...
function normalizeEntryInput({
  ...
  const peerSessionId = extractPeerSessionId(item, ownerReport)
  const remoteAgentId = deriveRemoteAgentId({ agentId, item, ownerReport })
  const normalizedSelectedSkill = extractSelectedSkill(selectedSkill, ownerReport)
  const normalizedOwnerReport = normalizeOwnerReport(ownerReport, {
    conversationKey,
    remoteAgentId,
  ...
  return {
    ...
    remoteAgentId,
    selectedSkill: normalizedSelectedSkill,
    item: mergeConversationMetadataIntoItem({
      ...(item ?? {}),
      remoteAgentId,
      peerSessionId
    }, conversationKey),
    ownerReport: normalizedOwnerReport
  }
}
...
function buildEntry({
  ...
  const id = clean(normalized.item?.inboundId) || clean(normalized.ownerReport?.deliveryId) || safeSegment(createdAt)
  const remoteAgentId = normalized.remoteAgentId
  const normalizedConversationKey = normalized.conversationKey
  const inboundText = extractInboundText(normalized.item) || extractOwnerReportMessageText(normalized.ownerReport)
  const replyText = extractReplyText(peerResponse) || extractOwnerReportReplyText(normalized.ownerReport)
  const normalizedSelectedSkill = normalized.selectedSkill
  const summary = clean(normalized.ownerReport?.summary) || `${remoteAgentId || 'unknown'} opened an AgentSquared ${normalizedSelectedSkill || 'conversation'} session.`
  ...
  return {
    ...
    peerSessionId: normalized.peerSessionId,
    selectedSkill: normalizedSelectedSkill,
    summary,
    messageExcerpt: excerpt(inboundText),
    replyExcerpt: excerpt(replyText),
    ownerReport: normalized.ownerReport,
    ownerDelivery: ownerDelivery ?? null,
    request: normalized.item?.request ?? null,
...
function isFinalOwnerReport(entry = null) {
  return Boolean(entry?.ownerReport?.final)
}
...
function needsOwnerReviewStatus(status = '') {
  return ['failed', 'timeout', 'maybe_sent'].includes(clean(status))
}
...
function createSqliteInboxStore({ inboxDir } = {}) {
  ...
  const apiUsageDbFile = path.join(resolvedInboxDir, 'api_usage.sqlite')
  ensureDir(resolvedInboxDir)

  const db = new DatabaseSync(dbFile)
  const apiUsageDb = new DatabaseSync(apiUsageDbFile)
  apiUsageDb.exec(`
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS inbox_entries (
      ...
      owner_report_final INTEGER DEFAULT 0,
  ...
  `)
  if (!hasColumn(db, 'inbox_entries', 'owner_report_final')) {
    db.exec('ALTER TABLE inbox_entries ADD COLUMN owner_report_final INTEGER DEFAULT 0;')
  }
  ...
  function findExisting({ id, dedupeKey, requestId, peerSessionId, finalReport, conversationKey }) {
    ...
    if (finalReport && clean(conversationKey)) {
      return rowToEntry(db.prepare('SELECT * FROM inbox_entries WHERE conversation_key = ? AND owner_report_final = 1 ORDER BY created_at DESC LIMIT 1').get(clean(conversationKey)))
    }
    return null
  }
  ...
  function appendEntry({
    ...
    const id = clean(normalized.item?.inboundId) || clean(normalized.ownerReport?.deliveryId) || safeSegment(createdAt)
    const requestId = extractRequestId(normalized.item, normalized.ownerReport)
    const conversationKey = normalized.conversationKey
    const dedupeKey = extractDedupeKey(normalized.item, normalized.ownerReport)
    const finalReport = Boolean(normalized.ownerReport?.final)
    ...
    const entry = buildEntry({
      ...
      file,
      agentId,
      selectedSkill: normalized.selectedSkill,
      mailboxKey,
      item: { ...normalized.item, inboundId: finalId },
      ownerReport: normalized.ownerReport,
      peerResponse,
      ownerDelivery
    })
    ...
    db.prepare(`
      INSERT INTO inbox_entries (
        ...
        owner_report_final, owner_delivery_json, owner_delivery_status,
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        owner_report_final = excluded.owner_report_final,
  ...
  function findFinalConversationReport(conversationKey = '') {
    ...
    return rowToEntry(db.prepare('SELECT * FROM inbox_entries WHERE conversation_key = ? AND owner_report_final = 1 ORDER BY created_at DESC LIMIT 1').get(targetConversationKey))
  }
  ...
  function findConversation(conversationKey = '') {
    ...
    const entries = db.prepare('SELECT * FROM inbox_entries WHERE conversation_key = ? ORDER BY created_at ASC').all(targetConversationKey).map(rowToEntry)
    const finalEntry = [...entries].reverse().find((entry) => Boolean(entry?.ownerReport?.final))
      ?? findFinalConversationReport(targetConversationKey)
      ?? entries.at(-1)
      ?? null
    if (!finalEntry) {
      return null
    ...
    return {
      ...
      finalEntry,
      entries,
      ownerReport: finalEntry.ownerReport ?? null,
      conversationTurns: finalEntry.ownerReport?.conversationTurns ?? [],
      summary: clean(finalEntry.ownerReport?.summary || finalEntry.summary)
    }
  }

  function snapshot() {
    const index = readIndex()
    const h2aRows = db.prepare(`
      SELECT conversation_key, COUNT(*) AS message_count, MAX(updated_at) AS last_activity_at
      FROM inbox_entries
      WHERE selected_skill = 'human-agent-chat'
    ...
    return {
      ...
      h2aUnreadSessions: h2aRows.length,
      h2aUnreadMessages: h2aRows.reduce((sum, row) => sum + (Number(row.message_count) || 0), 0),
  ...
  function appendApiUsage({
    ...
    const createdAt = nowISO()
    const input = Number.parseInt(`${inputTokens ?? 0}`, 10) || 0
    const output = Number.parseInt(`${outputTokens ?? 0}`, 10) || 0
    const cacheCreation = Number.parseInt(`${cacheCreationInputTokens ?? 0}`, 10) || 0
    const cacheRead = Number.parseInt(`${cacheReadInputTokens ?? 0}`, 10) || 0
    const total = input + output
    
    const recordId = clean(id) || `api-usage-${Date.now()}-${Math.random().toString(16).slice(2)}`
    
    apiUsageDb.prepare(`
      INSERT INTO api_usage_records (
        id, request_id, created_at, caller, skill,
  ...
  return {
    ...
    inboxMarkdownFile,
    sqliteFile: dbFile,
    apiUsageSqliteFile: apiUsageDbFile,
    backend: 'sqlite',
    readIndex,
    rebuildIndex,
    appendEntry,
