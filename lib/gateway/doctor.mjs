...
import { defaultInboxDir, defaultPeerKeyFile, defaultReceiptFile, resolveUserPath } from '../shared/paths.mjs'
...
async function probeCodexSmokeTurn({
  codexPath = '',
  timeoutMs = 45000
} = {}) {
  ...
  try {
    ...
    const result = await client.smokeTurn({
      timeoutMs: Math.max(1000, timeoutMs)
    })
    return {
      ok: true,
      ...
      textLength: clean(result?.text).length
    }
  } finally {
    await client.close()
  }
...
export async function runGatewayDoctor({
  ...
  if (resolvedContext) {
    try {
      ...
      try {
        const receiptFile = defaultReceiptFile(resolvedContext.keyFile, resolvedContext.agentId)
        const receipt = JSON.parse(fs.readFileSync(receiptFile, 'utf8'))
        const receiptPublicKey = clean(receipt?.publicKey)
        const matchesReceipt = receiptPublicKey && receiptPublicKey === clean(keyBundle.publicKey)
        checks.push(check(
          'identity.receipt',
          'Registration Receipt',
          matchesReceipt ? 'ok' : 'fail',
          matchesReceipt
            ? 'runtime key matches registration receipt'
            : 'runtime key does not match registration receipt',
          receiptFile,
          matchesReceipt ? null : action('restore-key', 'Restore the original runtime-key.json for this Agent ID. Do not rerun onboarding to generate a replacement key.'),
          {
            agentId: resolvedContext.agentId,
            receiptFile
          }
        ...
        const fix = action('restore-receipt', 'Restore registration-receipt.json from the active AgentSquared profile.')
        rememberAction(recommendedActions, fix)
        checks.push(check('identity.receipt', 'Registration Receipt', 'fail', 'registration receipt could not be read', safeError(error), fix))
      }
    } catch (error) {
  ...
  if (resolvedContext) {
    try {
      ...
      const expectedPeerKeyFile = defaultPeerKeyFile(resolvedContext.keyFile, resolvedContext.agentId)
      const statePeerKeyFile = clean(gateway.state?.peerKeyFile)
      const peerKeyFile = statePeerKeyFile || expectedPeerKeyFile
      const peerKeyExists = fs.existsSync(resolveUserPath(peerKeyFile))
      const statePeerID = clean(gateway.state?.peerId)
      const healthPeerID = clean(gateway.health?.peerId)
      const peerStateOK = peerKeyExists && (!statePeerID || !healthPeerID || statePeerID === healthPeerID)
      checks.push(check(
        'gateway.peer',
        'Gateway Peer Identity',
        peerStateOK ? 'ok' : 'fail',
        peerStateOK
          ? 'persistent gateway peer identity is present'
          : 'gateway peer identity is missing or inconsistent',
        peerKeyFile,
        peerStateOK ? null : action('restart-gateway', 'Restart the gateway so it uses the persistent runtime/gateway-peer.key for this Agent ID.'),
        {
          peerKeyFile,
          statePeerId: statePeerID,
          healthPeerId: healthPeerID
        }
      ))
    } catch (error) {
  ...
  const hostReady = Boolean(
    ...
    || detectedHostRuntime?.authHealthy
    || gateway?.health?.startupChecks?.hostRuntime?.ok
  )
  const hostDetail = clean(detectedHostRuntime?.authHint) || clean(detectedHostRuntime?.reason) || clean(gateway?.health?.startupChecks?.hostRuntime?.error)
  checks.push(check(
    'host.runtime',
    'Host Runtime',
  ...
  if (hostResolved === 'codex') {
    const smokeTimeoutMs = Math.max(1000, Number.parseInt(args['codex-smoke-timeout-ms'] ?? args['codex-timeout-ms'] ?? '45000', 10) || 45000)
    const codexPath = clean(detectedHostRuntime?.codexPath || args['codex-command'])
    try {
      const probe = await probeCodexSmokeTurn({
        codexPath,
      ...
      checks.push(check(
        ...
        'Codex Turn',
        'ok',
        `completed a real Codex turn in ${probe.durationMs}ms`,
        clean(codexPath) || 'default Codex binary',
        null,
        {
      ...
      const fix = action(
        'fix-codex-runtime',
        'Open Codex locally and confirm it can complete a short prompt, then rerun gateway doctor. Check Codex auth, network access, and model availability.'
      )
      rememberAction(recommendedActions, fix)
      checks.push(check(
        'host.codex-turn',
        'Codex Turn',
        'fail',
        'Codex adapter is reachable but cannot complete a real turn',
        safeError(error),
        fix,
        {
          timeoutMs: smokeTimeoutMs,
          codexPath: clean(codexPath)
        }
      ))
    }
  }
  ...
  if (resolvedContext && keyBundle) {
    ...
    try {
      ...
      checks.push(check(
        ...
        `${apiBase}/api/relay/following`,
        null,
        {
          followingCount: Array.isArray(directory?.items) ? directory.items.length : undefined
        }
      ))
    } catch (error) {
