...
const DEFAULT_PRESENCE_REFRESH_MS = 10 * 60 * 1000
...
export async function runGateway(argv) {
  ...
  const codexCommand = `${args['codex-command'] ?? process.env.CODEX_COMMAND ?? ''}`.trim()
  const codexTimeoutMs = Math.max(1000, Number.parseInt(args['codex-timeout-ms'] ?? `${process.env.CODEX_TIMEOUT_MS ?? '180000'}`, 10) || 180000)
  const claudeCommand = `${args['claude-command'] ?? process.env.CLAUDE_COMMAND ?? 'claude'}`.trim() || 'claude'
  const claudeCwd = `${args['claude-cwd'] ?? process.env.CLAUDE_CWD ?? ''}`.trim()
  const claudeModel = `${args['claude-model'] ?? process.env.CLAUDE_MODEL ?? ''}`.trim()
  ...
  const claudeSettingSources = `${args['claude-setting-sources'] ?? process.env.CLAUDE_SETTING_SOURCES ?? 'none'}`.trim() || 'none'
  ...
  const localRuntimeExecutor = createLocalRuntimeExecutor({
    ...
    hermesTimeoutMs,
    codexCommand: clean(detectedHostRuntime.codexPath) || codexCommand,
    codexTimeoutMs,
    claudeCommand: clean(detectedHostRuntime.claudeCommand) || claudeCommand,
    claudeCwd,
    claudeModel,
    claudeTimeoutMs,
    claudeMaxTurns,
  ...
  function isAgentAlreadyActiveError(error) {
    const message = `${error?.message ?? error ?? ''}`.toLowerCase()
    return error?.statusCode === 409 && (
      `${error?.errorCode ?? ''}`.toUpperCase() === 'AGENT_ALREADY_ACTIVE'
      || message.includes('already has an active agentsquared gateway')
    )
  }
  ...
  async function recoverGateway(reason) {
    ...
    recoveryPromise = (async () => {
      ...
      try {
        ...
        if (isAgentAlreadyActiveError(error)) {
          stopping = true
        }
