...
import { buildSenderBaseReport, peerResponseText } from '../conversation/templates.mjs'
...
export async function notifyLateConnectResult({
  ...
  const actionItems = shouldContinueConversation(conversation)
    ? ['The remote agent replied after the local wait window expired, so this live multi-turn exchange did not continue automatically. Ask me to continue it if you want to keep going.']
    : []
  const senderReport = buildSenderBaseReport({
    ...
    overallSummary: excerpt(replyText, 240) || 'The remote agent replied after the local wait window expired.',
    actionItems,
    turnOutline: [
      {
        ...
        summary: [
          ...
          replyText ? `the peer later replied "${excerpt(replyText, 120)}"` : 'the peer later replied',
          'after the local wait window had already expired'
        ].join(' ')
      }
    ],
    detailsHint: 'This final peer reply arrived after the original local command had already timed out. AgentSquared delivered it asynchronously through the local inbox/owner notification path.',
    language: 'en',
    timeZone: 'UTC',
    localTime: false
  })
    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/owner/notifications') {
        ...
        const result = await ownerNotifier({
          ...
          notifyOwnerNow: body.notifyOwnerNow !== false
        })
        jsonResponse(res, 200, {
          ok: true,
          handled: true,
