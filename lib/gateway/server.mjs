  ...
  async function notifyOutboundJobSuccess(job, result, turnLog, {
    ...
    const senderReport = buildSenderBaseReport({
      ...
      detailsHint: continuationError
        ? `Detailed turn-by-turn exchange is available in the local inbox. The local AI runtime then failed while preparing the next turn: ${continuationError}`
    ...
    try {
      while (!stopping) {
        ...
        result = await openDirectPeerSession({
          ...
          sessionStore: runtimeState,
          // Multi-turn jobs must not depend on a previously-open libp2p
          // connection. Half-open connections can accept newStream locally but
          // never reach the peer handler, which makes the sender wait for a
          // request receipt that will never arrive. Keep the trusted session as
          // conversation identity, but create a fresh relay-backed turn path.
