...
import { normalizeAgentSquaredAgentId } from '../shared/agent_id.mjs'
...
export async function notifyLateConnectResult({
  ...
  const targetAgentId = body.targetAgentId
    ? normalizeAgentSquaredAgentId(body.targetAgentId, { label: 'targetAgentId' })
    : ''
...
export async function runGateway(argv) {
  ...
  const agentId = normalizeAgentSquaredAgentId(requireArg(args['agent-id'], '--agent-id is required'), { label: '--agent-id' })
  ...
  function enqueueOutboundConversationJob(body = {}) {
    ...
    const job = {
      jobId,
      status: 'queued',
      targetAgentId: normalizeAgentSquaredAgentId(requireArg(body.targetAgentId, 'targetAgentId is required'), { label: 'targetAgentId' }),
