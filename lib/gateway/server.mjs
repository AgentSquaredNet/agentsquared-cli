...
import { buildSenderBaseReport, buildSenderFailureReport, inferOwnerFacingLanguage, peerResponseText } from '../conversation/templates.mjs'
...
export async function runGateway(argv) {
  ...
  async function ensureGatewayReady(reason) {
    ...
    try {
      const lastHealthy = lifecycle.lastHealthyAt ? Date.parse(lifecycle.lastHealthyAt) : 0
      const now = Date.now()
      const isFresh = Number.isFinite(lastHealthy) && (now - lastHealthy) < 60000

      if (activeOperations > 0 && isFresh && currentNode && currentNode.isStarted) {
        return currentNode
      }
  ...
  async function notifyOutboundJobSuccess(job, result, turnLog, {
    ...
    const senderReport = buildSenderBaseReport({
      ...
      overallSummary: excerpt(replyText || turnLog.at(-1)?.replyText || '', 240),
      turnOutline,
      actionItems: [],
      detailsHint: continuationError
        ? `Detailed turn-by-turn exchange is available in the local inbox. The exchange then failed: ${continuationError}`
        : 'Detailed turn-by-turn exchange is available in the local inbox.',
      language: job.ownerLanguage,
      timeZone: job.ownerTimeZone,
      localTime: true
    })
  ...
  async function runOutboundConversationJob(job) {
    ...
    try {
      ...
      while (!stopping) {
        ...
        logGatewayEvent('outbound-job-turn-start', {
          jobId: job.jobId,
          conversationKey: job.conversationKey,
          targetAgentId: job.targetAgentId,
          turnIndex,
        ...
        result = await openDirectPeerSession({
          ...
          metadata: {
            ...(job.sharedSkill ? { sharedSkill: job.sharedSkill } : {}),
        ...
        logGatewayEvent('outbound-job-turn-peer-finish', {
          jobId: job.jobId,
          conversationKey: job.conversationKey,
          targetAgentId: job.targetAgentId,
          turnIndex,
        ...
        logGatewayEvent('outbound-job-turn-local-start', {
          jobId: job.jobId,
          conversationKey: job.conversationKey,
          targetAgentId: job.targetAgentId,
        ...
        try {
          ...
          console.error(`${continuationError}${error.cause ? `; cause: ${error.cause.message || error.cause}` : ''}`, error)
          break
        }
        logGatewayEvent('outbound-job-turn-local-finish', {
          jobId: job.jobId,
          conversationKey: job.conversationKey,
          targetAgentId: job.targetAgentId,
          turnIndex: nextTurnIndex,
      ...
      logGatewayEvent('outbound-job-completed', {
        jobId: job.jobId,
        conversationKey: job.conversationKey,
        targetAgentId: job.targetAgentId,
        turnCount: job.turnCount,
  ...
  function enqueueOutboundConversationJob(body = {}) {
    ...
    const sharedSkill = metadata.sharedSkill && typeof metadata.sharedSkill === 'object' ? metadata.sharedSkill : null
