...
function createEventQueue() {
  ...
  function fail(error) {
    failure = error instanceof Error ? error : new Error(`${error ?? 'stream event queue failed'}`)
    closed = true
    while (waiters.length > 0) {
...
export function createGatewayRuntimeState({
  ...
  function forgetTrustedSession({
    ...
    const removeSession = (sessionId) => {
      const known = trustedSessions.get(sessionId)
      if (!known) {
        return
      }
      trustedSessions.delete(sessionId)
      const knownConversationKey = `${known.conversationKey ?? ''}`.trim()
      if (knownConversationKey && trustedByConversation.get(knownConversationKey) === sessionId) {
        trustedByConversation.delete(knownConversationKey)
      }
      if (!removed) {
        removed = clone(known)
      }
    ...
    if (normalizedConversationKey) {
      const mappedSessionId = trustedByConversation.get(normalizedConversationKey)
      if (mappedSessionId) {
  ...
  async function enqueueInbound({
    ...
    if (requestKey) {
      ...
      if (existingPending) {
        return {
          ...
          streamEvents: existingPending.eventQueue.iterator,
    ...
    const payload = {
      ...
      defaultSkill: `${defaultSkill}`.trim(),
      ticketView: ticketView ? clone(ticketView) : null,
    ...
    return {
      ...
      payload,
      responsePromise,
      streamEvents: eventQueue.iterator
    }
  }
  ...
  function emitInboundStreamEvent({ inboundId, event }) {
    pruneExpiredInbound()
    const pending = pendingInbound.get(`${inboundId}`.trim())
    if (!pending) {
      return false
    }
    return pending.eventQueue?.push?.(event) ?? false
  }

  function rejectInbound({
    inboundId,
    code = 500,
    message = 'local runtime rejected the inbound request',
    stage = '',
    runtimeAdapter = '',
    failureKind = '',
    detail = ''
    ...
    const normalizedCode = Number.parseInt(`${code ?? 500}`, 10) || 500
    const error = Object.assign(new Error(`${message}`), {
      code: normalizedCode,
      data: {
        kind: 'local-runtime-reject',
        failureKind: `${failureKind ?? ''}`.trim() || 'local-runtime-reject',
        stage: `${stage ?? ''}`.trim(),
