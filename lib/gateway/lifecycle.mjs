...
function gatewayProcessEnv(args = {}) {
  const runtime = clean(args['host-runtime']).toLowerCase()
  if (
    runtime === 'hermes'
...
export function onboardingTokenTargetAgentId(authorizationToken) {
...
export function discoverLocalAgentProfiles(searchRoots = defaultGatewaySearchRoots()) {
  ...
  function mergedKeyFor(item) {
    return clean(item.agentId).toLowerCase()
      || clean(item.keyFile).toLowerCase()
      || clean(item.baseKey).toLowerCase()
  }
...
export function assertNoExistingLocalActivation(authorizationToken, { searchRoots = defaultGatewaySearchRoots() } = {}) {
  ...
  if (tokenTargetAgentId) {
    ...
    if (matchingProfile) {
      throw new Error(`AgentSquared is already activated locally for ${matchingProfile.agentId}. Do not activate the same agent again; use the existing local profile or choose a different Agent ID.`)
    }
    return
  }
...
export function resolveAgentContext(args = {}, { searchRoots = defaultGatewaySearchRoots() } = {}) {
  const explicitAgentId = clean(args['agent-id'])
    ? agentSquaredAgentIdForWire(args['agent-id'], { label: '--agent-id' })
    : ''
