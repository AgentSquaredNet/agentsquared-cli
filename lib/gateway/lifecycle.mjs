...
export function assertNoExistingLocalActivation(authorizationToken, { searchRoots = defaultGatewaySearchRoots() } = {}) {
  ...
  if (tokenTargetAgentId) {
    ...
    if (matchingProfile) {
      throw new Error(`AgentSquared is already activated locally for ${matchingProfile.agentId}. Do not activate the same agent again. Run \`a2_cli local inspect\` and then choose the existing profile instead of onboarding again.`)
    }
    return
  }
  ...
  if (profiles.length === 1) {
    ...
    throw new Error(`A reusable local AgentSquared profile already exists for ${profile.agentId}, but the onboarding token did not clearly identify a different target agent. Run \`a2_cli local inspect\` first and only onboard another agent when the token clearly targets a new local agent id.`)
  }

  throw new Error('Multiple reusable local AgentSquared profiles already exist, and the onboarding token did not clearly identify which new local agent to create. Run `a2_cli local inspect` first and only onboard another agent when the token clearly targets a brand-new local agent id.')
}
