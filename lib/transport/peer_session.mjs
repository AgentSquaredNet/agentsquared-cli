...
export async function attachInboundRouter({
  ...
  node.handle(binding.streamProtocol, async (eventOrStream, maybeConnection) => {
    ...
    try {
      ...
      const conversationKey = `${metadata.conversationKey ?? h2aSessionId ?? ''}`.trim()
      if (!conversationKey) {
      ...
      if (isH2A && !relayConnectTicket) {
        await writeLine(stream, JSON.stringify({
          jsonrpc: '2.0',
          id: request?.id ?? randomRequestId('invalid'),
          error: { code: 401, message: 'H2A_TICKET_REQUIRED: relayConnectTicket is required for Human-to-Agent browser messages' }
        }))
        return
      }
      ...
      const inbound = await sessionStore.enqueueInbound({
        ...
        peerSessionId,
        suggestedSkill,
        defaultSkill: isH2A ? 'human-agent-chat' : ''
      })
