import { randomRequestId } from '../shared/primitives.mjs'
import { createConnectTicket, getAgentCard, introspectConnectTicket, postOnline, reportSession } from './relay_http.mjs'
import { currentPeerConnection, dialProtocol, openStreamOnExistingConnection, readJsonMessage, waitForPublishedTransport, writeLine } from './libp2p.mjs'

const RELAY_RECOVERY_RETRY_DELAY_MS = 1500
const RESPONSE_ACK_TIMEOUT_MS = 3 * 1000
const TURN_RECEIPT_TIMEOUT_MS = 10 * 1000
const TURN_RESPONSE_TIMEOUT_MS = 210 * 1000
const TURN_DIAL_TIMEOUT_MS = 60 * 1000

export function buildJsonRpcEnvelope({ id, method, message, metadata = {} }) {
  return {
    jsonrpc: '2.0',
    id: id ?? randomRequestId('a2a'),
    method,
    params: {
      message,
      metadata
    }
  }
}

function buildJsonRpcAck(id) {
  return {
    jsonrpc: '2.0',
    id,
    result: {
      ack: true
    }
  }
}

function buildJsonRpcReceipt(id) {
  return {
    jsonrpc: '2.0',
    id,
    result: {
      received: true
    }
  }
}

function buildJsonRpcStreamEvent(requestId, event = {}) {
  return {
    jsonrpc: '2.0',
    method: 'response/stream_event',
    params: {
      requestId,
      ...event
    }
  }
}

function isJsonRpcAck(message, id = '') {
  return Boolean(
    message
    && typeof message === 'object'
    && `${message.jsonrpc ?? ''}`.trim() === '2.0'
    && `${message.id ?? ''}`.trim() === `${id ?? ''}`.trim()
    && message.result
    && typeof message.result === 'object'
    && message.result.ack === true
  )
}

function isJsonRpcReceipt(message, id = '') {
  return Boolean(
    message
    && typeof message === 'object'
    && `${message.jsonrpc ?? ''}`.trim() === '2.0'
    && `${message.id ?? ''}`.trim() === `${id ?? ''}`.trim()
    && message.result
    && typeof message.result === 'object'
    && message.result.received === true
  )
}

function isJsonRpcStreamEvent(message, id = '') {
  if (!message || typeof message !== 'object') {
    return false
  }
  if (`${message.jsonrpc ?? ''}`.trim() !== '2.0') {
    return false
  }
  if (`${message.method ?? ''}`.trim() !== 'response/stream_event') {
    return false
  }
  const requestId = `${message.params?.requestId ?? ''}`.trim()
  return !requestId || requestId === `${id ?? ''}`.trim()
}

async function sendResponseAck(stream, requestId) {
  if (!stream || !`${requestId ?? ''}`.trim()) {
    return
  }
  try {
    await writeLine(stream, JSON.stringify(buildJsonRpcAck(requestId)))
  } catch {
    // best-effort only; the response itself was already received locally
  }
}

async function sendRequestReceipt(stream, requestId) {
  if (!stream || !`${requestId ?? ''}`.trim()) {
    return
  }
  await writeLine(stream, JSON.stringify(buildJsonRpcReceipt(requestId)))
}

async function waitForOptionalAck(stream, requestId, timeoutMs = RESPONSE_ACK_TIMEOUT_MS) {
  if (!stream || !`${requestId ?? ''}`.trim()) {
    return false
  }
  try {
    const ack = await Promise.race([
      readJsonMessage(stream),
      new Promise((resolve) => setTimeout(() => resolve(null), timeoutMs))
    ])
    return isJsonRpcAck(ack, requestId)
  } catch {
    return false
  }
}

async function readMessageWithTimeout(readMessageFn, stream, timeoutMs, label) {
  return operationWithTimeout(() => readMessageFn(stream), timeoutMs, label)
}

async function operationWithTimeout(run, timeoutMs, label) {
  const normalizedTimeoutMs = Math.max(250, Number.parseInt(`${timeoutMs ?? 0}`, 10) || TURN_DIAL_TIMEOUT_MS)
  let timeout = null
  try {
    return await Promise.race([
      run(),
      new Promise((_, reject) => {
        timeout = setTimeout(() => {
          reject(new Error(`${label} timed out after ${normalizedTimeoutMs}ms`))
        }, normalizedTimeoutMs)
      })
    ])
  } catch (error) {
    throw error
  } finally {
    if (timeout) {
      clearTimeout(timeout)
    }
  }
}

function emitDiagnostic(diagnostics, event, details = {}) {
  if (typeof diagnostics !== 'function') {
    return
  }
  try {
    diagnostics(event, details)
  } catch {
    // Diagnostics must never affect transport behavior.
  }
}

function diagnosticError(error = null) {
  return {
    message: cleanPeerSessionString(error?.message),
    code: cleanPeerSessionString(error?.code)
  }
}

function jsonRpcErrorData(error = null) {
  const data = error?.data && typeof error.data === 'object'
    ? error.data
    : error?.details && typeof error.details === 'object'
      ? error.details
      : null
  if (!data) {
    return null
  }
  const out = {}
  for (const [key, value] of Object.entries(data)) {
    if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
      out[key] = value
    }
  }
  return Object.keys(out).length > 0 ? out : null
}

function transportDiagnosticSummary(transport = null) {
  return {
    peerId: cleanPeerSessionString(transport?.peerId),
    dialAddrCount: Array.isArray(transport?.dialAddrs) ? transport.dialAddrs.length : 0,
    relayAddrCount: Array.isArray(transport?.relayAddrs) ? transport.relayAddrs.length : 0,
    listenAddrCount: Array.isArray(transport?.listenAddrs) ? transport.listenAddrs.length : 0,
    hasStreamProtocol: Boolean(cleanPeerSessionString(transport?.streamProtocol))
  }
}

function inferPostDispatchFailureKind(error = null) {
  const explicit = `${error?.a2FailureKind ?? ''}`.trim()
  if (explicit) {
    return explicit
  }
  const lower = describeError(error).toLowerCase()
  if (!lower) {
    return ''
  }
  if (lower.includes('turn response timed out after')) {
    return 'post-dispatch-response-timeout'
  }
  if (lower === 'empty json message') {
    return 'post-dispatch-empty-response'
  }
  if (
    lower.includes('stream that is closed')
    || lower.includes('stream closed before drain')
    || lower.includes('stream reset')
    || lower.includes('connection reset')
    || lower.includes('connection closed')
  ) {
    return 'post-dispatch-stream-closed'
  }
  return 'post-dispatch-response-unconfirmed'
}

export async function currentTransport(node, binding, options = {}) {
  return waitForPublishedTransport(node, binding, options)
}

export async function publishGatewayPresence(apiBase, agentId, bundle, node, binding, activitySummary, {
  availabilityStatus = 'available',
  requireRelayReservation = false
} = {}) {
  const transport = await currentTransport(node, binding, { requireRelayReservation })
  return postOnline(apiBase, agentId, bundle, {
    availabilityStatus,
    activitySummary,
    peerId: transport.peerId,
    listenAddrs: transport.listenAddrs,
    relayAddrs: transport.relayAddrs,
    supportedBindings: transport.supportedBindings,
    a2aProtocolVersion: transport.a2aProtocolVersion,
    streamProtocol: transport.streamProtocol
  })
}

export async function openDirectPeerSession({
  apiBase,
  agentId,
  bundle,
  node,
  binding,
  targetAgentId,
  skillName,
  method,
  message,
  metadata = null,
  activitySummary,
  report,
  sessionStore = null,
  allowTrustedReuse = true,
  diagnostics = null,
  _deps = null
}) {
  const deps = {
    currentPeerConnectionFn: currentPeerConnection,
    exchangeOverTransportFn: exchangeOverTransport,
    currentTransportFn: currentTransport,
    createConnectTicketWithRecoveryFn: createConnectTicketWithRecovery,
    ...(_deps && typeof _deps === 'object' ? _deps : {})
  }
  const metadataPayload = metadata && typeof metadata === 'object' ? metadata : {}
  const conversationKey = `${metadataPayload.conversationKey ?? ''}`.trim()
  if (!conversationKey) {
    throw Object.assign(new Error('conversationKey is required for outbound AgentSquared conversations'), { code: 400 })
  }
  // Each AgentSquared turn always opens a fresh libp2p stream.
  // When a trusted peer session is available, we only reuse the underlying
  // peer connection and session metadata, not the previous stream itself.
  const cachedSession = sessionStore?.trustedSessionByConversation?.(conversationKey)
    ?? null
  const liveConnection = cachedSession?.remotePeerId ? deps.currentPeerConnectionFn(node, cachedSession.remotePeerId) : null
  const trustedSessionHit = Boolean(cachedSession?.peerSessionId)

  let ticket = null
  let peerSessionId = `${cachedSession?.peerSessionId ?? ''}`.trim()
  let targetTransport = null
  let reusedPeerConnection = false
  let ambiguousTrustedDispatchError = null
  const reusableTransport = allowTrustedReuse && cachedSession
    ? mergeTargetTransport({
        primary: cachedSession.remoteTransport,
        secondary: cachedSession?.remotePeerId
          ? {
              peerId: cachedSession.remotePeerId,
              streamProtocol: binding.streamProtocol
            }
          : null,
        streamProtocol: binding.streamProtocol
      })
    : null
  emitDiagnostic(diagnostics, 'session-state', {
    conversationKey,
    peerSessionId,
    trustedSessionHit,
    livePeerConnectionHit: Boolean(liveConnection),
    reusableTransportHit: Boolean(reusableTransport?.peerId && reusableTransport?.streamProtocol),
    cachedRemotePeerId: cleanPeerSessionString(cachedSession?.remotePeerId),
    reusableTransport: transportDiagnosticSummary(reusableTransport)
  })
  const requestId = randomRequestId('a2a')
  const buildRequest = ({
    relayConnectTicket = '',
    peerSessionId: nextPeerSessionId = ''
  } = {}) => buildJsonRpcEnvelope({
    id: requestId,
    method,
    message,
    metadata: {
      ...metadataPayload,
      relayConnectTicket,
      peerSessionId: `${nextPeerSessionId ?? ''}`.trim(),
      skillHint: `${skillName ?? ''}`.trim(),
      from: agentId,
      to: targetAgentId
    }
  })

  if (reusableTransport?.peerId && reusableTransport?.streamProtocol) {
    const trustedStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'trusted-session-start', {
      peerSessionId,
      reuseExistingConnection: Boolean(liveConnection),
      transport: transportDiagnosticSummary(reusableTransport)
    })
    try {
      if (cachedSession?.peerSessionId) {
        sessionStore?.touchTrustedSession?.(cachedSession.peerSessionId)
      }
      const reusedResponse = await deps.exchangeOverTransportFn({
        apiBase,
        agentId,
        bundle,
        binding,
        node,
        transport: reusableTransport,
        request: buildRequest({
          relayConnectTicket: '',
          peerSessionId
        }),
        reuseExistingConnection: Boolean(liveConnection),
        diagnostics
      })
      targetTransport = reusableTransport
      reusedPeerConnection = Boolean(liveConnection)
      emitDiagnostic(diagnostics, 'trusted-session-finish', {
        peerSessionId,
        durationMs: Date.now() - trustedStartedAt,
        reusedPeerConnection,
        transport: transportDiagnosticSummary(targetTransport)
      })

      if (peerSessionId && targetTransport?.peerId) {
        sessionStore?.rememberTrustedSession?.({
          peerSessionId,
          conversationKey,
          remoteAgentId: targetAgentId,
          remotePeerId: targetTransport.peerId,
          remoteTransport: targetTransport,
          skillHint: `${skillName ?? ''}`.trim()
        })
      }

      return {
        ticket,
        peerSessionId,
        response: reusedResponse,
        sessionReport: null,
        reusedPeerConnection,
        trustedSessionHit,
        usedTrustedSession: true,
        reusedTrustedSession: true,
        usedFreshRelayTicket: false,
        // Backward-compatible alias for existing callers.
        reusedSession: reusedPeerConnection
      }
    } catch (error) {
      const failureKind = cleanPeerSessionString(inferPostDispatchFailureKind(error))
      if (failureKind === 'post-dispatch-empty-response') {
        invalidateTrustedConversation(sessionStore, {
          peerSessionId: cachedSession?.peerSessionId || peerSessionId,
          conversationKey,
          diagnostics,
          reason: failureKind
        })
      }
      emitDiagnostic(diagnostics, 'trusted-session-error', {
        peerSessionId,
        durationMs: Date.now() - trustedStartedAt,
        dispatchStage: cleanPeerSessionString(error?.a2DispatchStage),
        deliveryStatusKnown: Boolean(error?.a2DeliveryStatusKnown),
        failureKind,
        ...diagnosticError(error)
      })
      if (!error?.a2DeliveryStatusKnown && `${error?.a2DispatchStage ?? ''}` === 'post-dispatch') {
        ambiguousTrustedDispatchError = withDeliveryStatusUnknown(error, inferPostDispatchFailureKind(error))
      } else if (!isTrustedSessionRetryable(error)) {
        throw error
      }
    }
  }

  let response
  try {
    let transport
    const currentTransportStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'current-transport-start', {
      requireRelayReservation: true
    })
    try {
      transport = await deps.currentTransportFn(node, binding, { requireRelayReservation: true })
      emitDiagnostic(diagnostics, 'current-transport-finish', {
        durationMs: Date.now() - currentTransportStartedAt,
        recovered: false,
        transport: transportDiagnosticSummary(transport)
      })
    } catch (transportError) {
      emitDiagnostic(diagnostics, 'current-transport-error', {
        durationMs: Date.now() - currentTransportStartedAt,
        recovered: false,
        ...diagnosticError(transportError)
      })
      // Relay reservation may have expired during a long multi-turn conversation.
      // Re-publish presence to re-establish the relay reservation before giving up.
      await bestEffortRepublishPresence(apiBase, agentId, bundle, node, binding)
      const recoveredTransportStartedAt = Date.now()
      emitDiagnostic(diagnostics, 'current-transport-retry-start', {
        requireRelayReservation: true
      })
      transport = await deps.currentTransportFn(node, binding, { requireRelayReservation: true })
      emitDiagnostic(diagnostics, 'current-transport-finish', {
        durationMs: Date.now() - recoveredTransportStartedAt,
        recovered: true,
        transport: transportDiagnosticSummary(transport)
      })
    }
    const ticketStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'connect-ticket-start', {
      cachedTransport: transportDiagnosticSummary(cachedSession?.remoteTransport),
      localTransport: transportDiagnosticSummary(transport)
    })
    let relayAttempt
    try {
      relayAttempt = await deps.createConnectTicketWithRecoveryFn({
        apiBase,
        agentId,
        bundle,
        node,
        binding,
        targetAgentId,
        skillName,
        transport,
        cachedTransport: cachedSession?.remoteTransport ?? null,
        diagnostics
      })
    } catch (error) {
      emitDiagnostic(diagnostics, 'connect-ticket-error', {
        durationMs: Date.now() - ticketStartedAt,
        ...diagnosticError(error)
      })
      throw error
    }
    ticket = relayAttempt.ticket
    targetTransport = relayAttempt.targetTransport
    emitDiagnostic(diagnostics, 'connect-ticket-finish', {
      durationMs: Date.now() - ticketStartedAt,
      targetTransport: transportDiagnosticSummary(targetTransport),
      hasTicket: Boolean(ticket?.ticket)
    })
    peerSessionId = peerSessionId || parseConnectTicketId(ticket.ticket) || randomRequestId('peer')

    response = await deps.exchangeOverTransportFn({
      apiBase,
      agentId,
      bundle,
      binding,
      node,
      transport: targetTransport,
      request: buildRequest({
        relayConnectTicket: ticket?.ticket ?? '',
        peerSessionId
      }),
      diagnostics
    })
  } catch (error) {
    emitDiagnostic(diagnostics, 'fresh-relay-error', {
      peerSessionId,
      dispatchStage: cleanPeerSessionString(error?.a2DispatchStage),
      deliveryStatusKnown: Boolean(error?.a2DeliveryStatusKnown),
      ...diagnosticError(error)
    })
    if (ambiguousTrustedDispatchError && !ambiguousTrustedDispatchError.a2DeliveryStatusKnown) {
      const followUpDetail = cleanPeerSessionString(error?.message)
      if (followUpDetail && !cleanPeerSessionString(ambiguousTrustedDispatchError.message).includes(followUpDetail)) {
        ambiguousTrustedDispatchError = withAppendedErrorDetail(
          ambiguousTrustedDispatchError,
          `Fresh relay retry also failed: ${followUpDetail}`
        )
      }
      throw ambiguousTrustedDispatchError
    }
    throw error
  }

  if (peerSessionId && targetTransport?.peerId) {
    sessionStore?.rememberTrustedSession?.({
      peerSessionId,
      conversationKey,
      remoteAgentId: targetAgentId,
      remotePeerId: targetTransport.peerId,
      remoteTransport: targetTransport,
      skillHint: `${skillName ?? ''}`.trim()
    })
  }

  let sessionReport = null
  if (report && ticket?.ticket) {
    const reportStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'session-report-start', {
      peerSessionId,
      taskId: cleanPeerSessionString(report.taskId)
    })
    try {
      sessionReport = await reportSession(apiBase, agentId, bundle, {
        ticket: ticket.ticket,
        taskId: report.taskId,
        status: report.status ?? 'completed',
        summary: report.summary,
        publicSummary: report.publicSummary ?? ''
      }, await bestEffortCurrentTransport(node, binding))
      emitDiagnostic(diagnostics, 'session-report-finish', {
        peerSessionId,
        durationMs: Date.now() - reportStartedAt
      })
    } catch (error) {
      emitDiagnostic(diagnostics, 'session-report-error', {
        peerSessionId,
        durationMs: Date.now() - reportStartedAt,
        ...diagnosticError(error)
      })
      throw error
    }
  }

  return {
    ticket,
    peerSessionId,
    response,
    sessionReport,
    reusedPeerConnection,
    trustedSessionHit,
    usedTrustedSession: false,
    reusedTrustedSession: false,
    usedFreshRelayTicket: Boolean(ticket?.ticket),
    reusedSession: reusedPeerConnection
  }
}

export async function createConnectTicketWithRecovery({
  apiBase,
  agentId,
  bundle,
  node,
  binding,
  targetAgentId,
  skillName,
  transport,
  cachedTransport = null,
  republishPresence = null,
  retryDelayMs = RELAY_RECOVERY_RETRY_DELAY_MS,
  diagnostics = null
}) {
  const attempt = async (attemptIndex = 0) => {
    const agentCardStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'connect-ticket-agent-card-start', {
      attempt: attemptIndex
    })
    const latestAgentCard = await bestEffortAgentCard(apiBase, agentId, bundle, targetAgentId, transport)
    emitDiagnostic(diagnostics, 'connect-ticket-agent-card-finish', {
      attempt: attemptIndex,
      durationMs: Date.now() - agentCardStartedAt,
      hasPreferredTransport: Boolean(latestAgentCard?.preferredTransport)
    })
    const createStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'connect-ticket-create-start', {
      attempt: attemptIndex
    })
    const ticket = await createConnectTicket(apiBase, agentId, bundle, targetAgentId, skillName, transport)
    emitDiagnostic(diagnostics, 'connect-ticket-create-finish', {
      attempt: attemptIndex,
      durationMs: Date.now() - createStartedAt,
      hasTicket: Boolean(ticket?.ticket)
    })
    const targetTransport = mergeTargetTransport({
      primary: latestAgentCard?.preferredTransport ?? null,
      secondary: ticket.targetTransport ?? ticket.agentCard?.preferredTransport ?? null,
      tertiary: cachedTransport,
      streamProtocol: binding.streamProtocol
    })
    return { ticket, targetTransport }
  }

  try {
    return await attempt(0)
  } catch (error) {
    emitDiagnostic(diagnostics, 'connect-ticket-attempt-error', {
      attempt: 0,
      retryable: isRelayPresenceRetryable(error),
      ...diagnosticError(error)
    })
    if (!isRelayPresenceRetryable(error)) {
      throw error
    }
    const recoveryStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'connect-ticket-recovery-start', {
      retryDelayMs
    })
    if (typeof republishPresence === 'function') {
      await republishPresence(error)
    } else {
      await bestEffortRepublishPresence(apiBase, agentId, bundle, node, binding)
    }
    await sleep(retryDelayMs)
    emitDiagnostic(diagnostics, 'connect-ticket-recovery-finish', {
      durationMs: Date.now() - recoveryStartedAt
    })
    return attempt(1)
  }
}

export async function attachInboundRouter({
  apiBase,
  agentId,
  bundle,
  node,
  binding,
  handler,
  sessionStore
}) {
  node.handle(binding.streamProtocol, async (eventOrStream, maybeConnection) => {
    const { stream, connection } = normalizeInboundStreamContext(eventOrStream, maybeConnection)
    const remotePeerId = connection?.remotePeer?.toString?.()
      ?? stream?.stat?.connection?.remotePeer?.toString?.()
      ?? ''
    let request = null
    let peerSessionId = ''
    let receiptSent = false
    try {
      request = await readJsonMessage(stream)
      const metadata = request?.params?.metadata ?? {}
      const channelKind = `${metadata.channelKind ?? ''}`.trim().toLowerCase()
      const h2aSessionId = `${metadata.h2aSessionId ?? ''}`.trim()
      const apiSessionId = `${metadata.apiSessionId ?? ''}`.trim()
      const conversationKey = `${metadata.conversationKey ?? (h2aSessionId || apiSessionId || '')}`.trim()
      if (!conversationKey) {
        await writeLine(stream, JSON.stringify({
          jsonrpc: '2.0',
          id: request?.id ?? randomRequestId('invalid'),
          error: { code: 400, message: 'conversationKey or h2aSessionId is required for inbound AgentSquared conversations' }
        }))
        return
      }
      await sendRequestReceipt(stream, request?.id)
      receiptSent = true
      const relayConnectTicket = `${metadata.relayConnectTicket ?? ''}`.trim()
      const requestedPeerSessionId = `${metadata.peerSessionId ?? ''}`.trim()
      let ticketView = null
      peerSessionId = requestedPeerSessionId
      let remoteAgentId = `${metadata.from ?? ''}`.trim()
      let suggestedSkill = `${metadata.skillHint ?? ''}`.trim()
      const isH2A = channelKind === 'h2a'
      const isAPI = channelKind === 'api'

      if ((isH2A || isAPI) && !relayConnectTicket) {
        await writeLine(stream, JSON.stringify({
          jsonrpc: '2.0',
          id: request?.id ?? randomRequestId('invalid'),
          error: { code: 401, message: `${isAPI ? 'API' : 'H2A'}_TICKET_REQUIRED: relayConnectTicket is required for ${isAPI ? 'Agent API' : 'Human-to-Agent browser'} messages` }
        }))
        return
      }

      if (relayConnectTicket) {
        ticketView = await introspectConnectTicket(
          apiBase,
          agentId,
          bundle,
          relayConnectTicket,
          await bestEffortCurrentTransport(node, binding)
        )
        peerSessionId = peerSessionId || ticketView.ticketId
        if (isH2A) {
          if (`${ticketView.scope ?? ''}`.trim() !== 'human-browser:connect-ticket') {
            await writeLine(stream, JSON.stringify({
              jsonrpc: '2.0',
              id: request?.id ?? randomRequestId('invalid'),
              error: { code: 401, message: 'H2A_SCOPE_INVALID: connect ticket is not scoped for Human-to-Agent browser chat' }
            }))
            return
          }
          if (`${ticketView.h2aSessionId ?? ''}`.trim() && `${ticketView.h2aSessionId ?? ''}`.trim() !== h2aSessionId) {
            await writeLine(stream, JSON.stringify({
              jsonrpc: '2.0',
              id: request?.id ?? randomRequestId('invalid'),
              error: { code: 403, message: 'H2A_SESSION_MISMATCH: message session does not match connect ticket' }
            }))
            return
          }
          const browserBridge = `${metadata.browserBridge ?? ''}`.trim()
          const browserBridgePeerId = `${metadata.browserBridgePeerId ?? ''}`.trim()
          const trustedWebServerBridge = browserBridge === 'agentsquared-webserver' && browserBridgePeerId && browserBridgePeerId === remotePeerId
          if (`${ticketView.browserPeerId ?? ''}`.trim() && remotePeerId && `${ticketView.browserPeerId ?? ''}`.trim() !== remotePeerId && !trustedWebServerBridge) {
            await writeLine(stream, JSON.stringify({
              jsonrpc: '2.0',
              id: request?.id ?? randomRequestId('invalid'),
              error: { code: 403, message: 'H2A_BROWSER_MISMATCH: browser peer does not match connect ticket' }
            }))
            return
          }
          remoteAgentId = remoteAgentId || `human:${ticketView.initiatorHumanName || ticketView.initiatorHumanId || 'browser'}`
          suggestedSkill = suggestedSkill || `${ticketView.skillName ?? ''}`.trim() || 'human-agent-chat'
        } else if (isAPI) {
          if (`${ticketView.scope ?? ''}`.trim() !== 'api:connect-ticket') {
            await writeLine(stream, JSON.stringify({
              jsonrpc: '2.0',
              id: request?.id ?? randomRequestId('invalid'),
              error: { code: 401, message: 'API_SCOPE_INVALID: connect ticket is not scoped for Agent API calls' }
            }))
            return
          }
          if (`${ticketView.apiSessionId ?? ''}`.trim() && `${ticketView.apiSessionId ?? ''}`.trim() !== apiSessionId) {
            await writeLine(stream, JSON.stringify({
              jsonrpc: '2.0',
              id: request?.id ?? randomRequestId('invalid'),
              error: { code: 403, message: 'API_SESSION_MISMATCH: message session does not match connect ticket' }
            }))
            return
          }
          remoteAgentId = remoteAgentId || `human:${ticketView.initiatorHumanName || ticketView.initiatorHumanId || 'api'}`
          suggestedSkill = suggestedSkill || `${ticketView.skillName ?? ''}`.trim() || 'human-agent-chat'
        } else if (`${ticketView.scope ?? ''}`.trim() && `${ticketView.scope ?? ''}`.trim() !== 'agent:connect-ticket') {
          await writeLine(stream, JSON.stringify({
            jsonrpc: '2.0',
            id: request?.id ?? randomRequestId('invalid'),
            error: { code: 401, message: 'CONNECT_TICKET_SCOPE_INVALID: connect ticket is not scoped for Agent-to-Agent messaging' }
          }))
          return
        } else {
          remoteAgentId = remoteAgentId || ticketView.initiatorAgentId
        }
        suggestedSkill = suggestedSkill || `${ticketView.skillName ?? ''}`.trim()
        const remoteTransport = buildInboundRemoteTransport({
          connection,
          remotePeerId,
          binding
        })
        sessionStore?.rememberTrustedSession?.({
          peerSessionId,
          conversationKey,
          remoteAgentId,
          remotePeerId,
          remoteTransport,
          ticketView,
          skillHint: suggestedSkill
        })
      } else {
        const trustedSession = sessionStore?.trustedSessionById?.(peerSessionId)
        if (!trustedSession || trustedSession.remotePeerId !== remotePeerId) {
          await writeLine(stream, JSON.stringify({
            jsonrpc: '2.0',
            id: request?.id ?? randomRequestId('invalid'),
            error: { code: 401, message: 'relayConnectTicket or a trusted peerSessionId is required' }
          }))
          return
        }
        sessionStore?.touchTrustedSession?.(trustedSession.peerSessionId)
        if (conversationKey) {
          sessionStore?.rememberTrustedSession?.({
            peerSessionId: trustedSession.peerSessionId,
            conversationKey,
            remoteAgentId: trustedSession.remoteAgentId,
            remotePeerId: trustedSession.remotePeerId,
            remoteTransport: trustedSession.remoteTransport,
            ticketView: trustedSession.ticketView,
            skillHint: trustedSession.skillHint
          })
        }
        remoteAgentId = remoteAgentId || trustedSession.remoteAgentId
        ticketView = trustedSession.ticketView ?? null
        suggestedSkill = suggestedSkill || trustedSession.skillHint || ''
      }

      const cachedHandledResponse = sessionStore?.handledRequestResponse?.(peerSessionId, request?.id)
      if (cachedHandledResponse) {
        await writeLine(stream, JSON.stringify(cachedHandledResponse))
        await waitForOptionalAck(stream, request?.id)
        return
      }

      const normalizedRequest = (isH2A || isAPI)
        ? {
            ...request,
            params: {
              ...(request?.params ?? {}),
              metadata: {
                ...(request?.params?.metadata ?? {}),
                conversationKey,
                channelKind: isAPI ? 'api' : 'h2a',
                ...(isAPI
                  ? {
                      apiSessionId,
                      apiRequestId: metadata.apiRequestId ?? ticketView?.apiRequestId ?? '',
                      apiKeyId: metadata.apiKeyId ?? ticketView?.apiKeyId ?? '',
                      source: metadata.source ?? ticketView?.source ?? 'openai-compatible-api',
                      sessionPolicy: metadata.sessionPolicy ?? { persistence: 'ephemeral', cleanup: 'best_effort' },
                      contextPolicy: metadata.contextPolicy ?? { owner: 'api_caller', persistence: 'stateless', retention: 'usage_only' }
                    }
                  : {
                      h2aSessionId,
                      h2aFirstMessage: Boolean(metadata.h2aFirstMessage ?? ticketView?.firstMessage),
                      sessionPolicy: metadata.sessionPolicy ?? { persistence: 'ephemeral', cleanup: 'best_effort' },
                      contextPolicy: metadata.contextPolicy ?? { owner: 'browser', persistence: 'stateless', retention: 'usage_only' }
                    }),
                fromHuman: metadata.fromHuman ?? ticketView?.initiatorHumanName ?? '',
                toAgent: metadata.toAgent ?? agentId,
                skillHint: suggestedSkill || 'human-agent-chat'
              }
            }
          }
        : request
      const inbound = await sessionStore.enqueueInbound({
        request: normalizedRequest,
        ticketView,
        remotePeerId,
        remoteAgentId,
        peerSessionId,
        suggestedSkill,
        defaultSkill: (isAPI || isH2A) ? 'human-agent-chat' : ''
      })
      let streamSequence = 1
      let result
      let done = false
      const eventIterator = inbound.streamEvents?.[Symbol.asyncIterator]?.()
      const responsePromise = inbound.responsePromise.then(
        (value) => ({ kind: 'response', value }),
        (error) => ({ kind: 'response-error', error })
      )
      let eventPromise = eventIterator
        ? eventIterator.next().then(
            (value) => ({ kind: 'event', value }),
            (error) => ({ kind: 'event-error', error })
          )
        : null
      while (!done) {
        const candidates = eventPromise ? [responsePromise, eventPromise] : [responsePromise]
        const next = await Promise.race(candidates)
        if (next.kind === 'response-error') {
          throw next.error
        }
        if (next.kind === 'response') {
          result = next.value
          done = true
          continue
        }
        if (next.kind === 'event-error') {
          eventPromise = null
          continue
        }
        const eventResult = next.value
        if (!eventResult?.done && eventResult?.value) {
          await writeLine(stream, JSON.stringify(buildJsonRpcStreamEvent(request?.id, {
            sequence: eventResult.value.sequence ?? streamSequence,
            createdAt: eventResult.value.createdAt ?? new Date().toISOString(),
            ...eventResult.value
          })))
          streamSequence += 1
        }
        eventPromise = eventIterator
          ? eventIterator.next().then(
              (value) => ({ kind: 'event', value }),
              (error) => ({ kind: 'event-error', error })
            )
          : null
      }
      const finalResult = typeof result === 'object' && result != null
        ? {
            ...result,
            metadata: {
              ...(result.metadata ?? {}),
              peerSessionId
            }
          }
        : { value: result, metadata: { peerSessionId } }
      const finalResponse = {
        jsonrpc: '2.0',
        id: request.id,
        result: finalResult
      }
      sessionStore?.rememberHandledRequest?.({
        peerSessionId,
        requestId: request?.id,
        response: finalResponse
      })
      await writeLine(stream, JSON.stringify(finalResponse))
      await waitForOptionalAck(stream, request?.id)
    } catch (error) {
      const data = jsonRpcErrorData(error)
      const errorResponse = {
        jsonrpc: '2.0',
        id: request?.id ?? randomRequestId('error'),
        error: {
          code: Number.parseInt(`${error.code ?? 500}`, 10) || 500,
          message: error.message,
          ...(data ? { data } : {})
        }
      }
      if (peerSessionId && request?.id && !sessionStore?.handledRequestResponse?.(peerSessionId, request.id)) {
        sessionStore?.rememberHandledRequest?.({
          peerSessionId,
          requestId: request.id,
          response: errorResponse
        })
      }
      if (request?.id && !receiptSent) {
        await sendRequestReceipt(stream, request?.id)
        receiptSent = true
      }
      await writeLine(stream, JSON.stringify(errorResponse))
      await waitForOptionalAck(stream, request?.id)
    } finally {
      await stream.close()
    }
  }, { runOnLimitedConnection: true })
}

function normalizeInboundStreamContext(eventOrStream, maybeConnection) {
  if (maybeConnection) {
    return {
      stream: eventOrStream,
      connection: maybeConnection
    }
  }

  return {
    stream: eventOrStream?.stream ?? eventOrStream,
    connection: eventOrStream?.connection ?? null
  }
}

async function bestEffortRepublishPresence(apiBase, agentId, bundle, node, binding) {
  try {
    await publishGatewayPresence(apiBase, agentId, bundle, node, binding, 'Refreshing relay presence after a transient delivery failure.', {
      requireRelayReservation: false
    })
  } catch {
    // best-effort only; retry path will still surface the original relay error if recovery did not help
  }
}

async function bestEffortCurrentTransport(node, binding) {
  try {
    return await currentTransport(node, binding)
  } catch {
    return null
  }
}

export async function exchangeOverTransport({
  apiBase = '',
  agentId = '',
  bundle = null,
  binding = null,
  node,
  transport,
  request,
  reuseExistingConnection = false,
  openStreamFn = openTransportStream,
  writeLineFn = writeLine,
  readMessageFn = readJsonMessage,
  turnDialTimeoutMs = TURN_DIAL_TIMEOUT_MS,
  turnReceiptTimeoutMs = TURN_RECEIPT_TIMEOUT_MS,
  turnResponseTimeoutMs = TURN_RESPONSE_TIMEOUT_MS,
  diagnostics = null,
  onStreamEvent = null
}) {
  let lastError = null
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const attemptStartedAt = Date.now()
    let stream = null
    let dispatchStage = 'pre-dispatch'
    let receiptConfirmed = false
    const previousFailureKind = cleanPeerSessionString(inferPostDispatchFailureKind(lastError))
    const disableExistingConnectionReuse = attempt > 0 && previousFailureKind === 'post-dispatch-empty-response'
    const attemptReuseExistingConnection = reuseExistingConnection && !disableExistingConnectionReuse
    try {
      emitDiagnostic(diagnostics, 'exchange-attempt-start', {
        attempt,
        reuseExistingConnection: attemptReuseExistingConnection,
        disableExistingConnectionReuse,
        allowDialFallback: attempt > 0
      })
      if (attempt > 0 && inferPostDispatchFailureKind(lastError).startsWith('post-dispatch-')) {
        // If the previous attempt failed due to a transport reset/timeout,
        // proactively refresh our own presence to re-establish potentially dead
        // relay reservations before the next dial attempt.
        if (apiBase && agentId && bundle && binding) {
          const republishStartedAt = Date.now()
          emitDiagnostic(diagnostics, 'republish-presence-start', {
            attempt
          })
          await bestEffortRepublishPresence(apiBase, agentId, bundle, node, binding)
          emitDiagnostic(diagnostics, 'republish-presence-finish', {
            attempt,
            durationMs: Date.now() - republishStartedAt
          })
        }
      }
      const streamStartedAt = Date.now()
      stream = await operationWithTimeout(
        () => openStreamFn(node, transport, {
          reuseExistingConnection: attemptReuseExistingConnection,
          allowDialFallback: attempt > 0,
          turnDialTimeoutMs,
          diagnostics
        }),
        turnDialTimeoutMs,
        'turn dial'
      )
      emitDiagnostic(diagnostics, 'exchange-stream-ready', {
        attempt,
        durationMs: Date.now() - streamStartedAt
      })
      const writeStartedAt = Date.now()
      await writeLineFn(stream, JSON.stringify(request))
      emitDiagnostic(diagnostics, 'exchange-write-finish', {
        attempt,
        durationMs: Date.now() - writeStartedAt
      })
      const receiptStartedAt = Date.now()
      const firstMessage = await readMessageWithTimeout(
        readMessageFn,
        stream,
        turnReceiptTimeoutMs,
        'request receipt'
      )
      emitDiagnostic(diagnostics, 'exchange-first-message-finish', {
        attempt,
        durationMs: Date.now() - receiptStartedAt,
        isReceipt: isJsonRpcReceipt(firstMessage, request?.id),
        isAck: isJsonRpcAck(firstMessage, request?.id)
      })
      let response = firstMessage
      if (isJsonRpcReceipt(firstMessage, request?.id)) {
        receiptConfirmed = true
        dispatchStage = 'post-dispatch'
        const responseStartedAt = Date.now()
        for (;;) {
          const nextMessage = await readMessageWithTimeout(
            readMessageFn,
            stream,
            turnResponseTimeoutMs,
            'turn response'
          )
          if (isJsonRpcStreamEvent(nextMessage, request?.id)) {
            await onStreamEvent?.(nextMessage.params ?? {})
            continue
          }
          response = nextMessage
          break
        }
        emitDiagnostic(diagnostics, 'exchange-response-finish', {
          attempt,
          durationMs: Date.now() - responseStartedAt
        })
      } else if (isJsonRpcAck(firstMessage, request?.id)) {
        const error = new Error('unexpected response acknowledgement before request receipt')
        error.a2DeliveryStatusKnown = true
        throw error
      } else {
        receiptConfirmed = true
        dispatchStage = 'post-dispatch'
      }
      await sendResponseAck(stream, request?.id)
      if (response.error) {
        throw buildJsonRpcError(response.error)
      }
      emitDiagnostic(diagnostics, 'exchange-attempt-finish', {
        attempt,
        dispatchStage,
        receiptConfirmed,
        durationMs: Date.now() - attemptStartedAt
      })
      return response
    } catch (error) {
      error.a2DispatchStage = error.a2DispatchStage || dispatchStage
      emitDiagnostic(diagnostics, 'exchange-attempt-error', {
        attempt,
        dispatchStage,
        receiptConfirmed,
        deliveryStatusKnown: Boolean(error?.a2DeliveryStatusKnown),
        failureKind: cleanPeerSessionString(inferPostDispatchFailureKind(error)),
        durationMs: Date.now() - attemptStartedAt,
        ...diagnosticError(error)
      })
      if (shouldRetryBeforeReceipt(error, attempt, receiptConfirmed)) {
        lastError = error
        continue
      }
      if (shouldRetryEmptyPostDispatch(error, attempt)) {
        lastError = error
        continue
      }
      if (lastError && `${lastError?.a2DispatchStage ?? ''}` === 'post-dispatch' && !lastError?.a2DeliveryStatusKnown) {
        throw withDeliveryStatusUnknown(lastError, inferPostDispatchFailureKind(lastError))
      }
      if (dispatchStage === 'post-dispatch' && !error.a2DeliveryStatusKnown) {
        throw withDeliveryStatusUnknown(error, inferPostDispatchFailureKind(error))
      }
      throw error
    } finally {
      await stream?.close?.()
    }
  }

  if (lastError) {
    throw withDeliveryStatusUnknown(lastError, inferPostDispatchFailureKind(lastError))
  }

  throw new Error('delivery status is unknown after the request was dispatched: response could not be confirmed')
}

async function openTransportStream(node, transport, {
  reuseExistingConnection = false,
  allowDialFallback = false,
  turnDialTimeoutMs = TURN_DIAL_TIMEOUT_MS,
  diagnostics = null
} = {}) {
  if (!reuseExistingConnection) {
    return dialProtocol(node, transport, {
      requireDirect: false,
      streamOpenTimeoutMs: turnDialTimeoutMs,
      diagnostics
    })
  }
  try {
    return await openStreamOnExistingConnection(node, transport, {
      streamOpenTimeoutMs: turnDialTimeoutMs,
      diagnostics
    })
  } catch (error) {
    if (!allowDialFallback) {
      throw error
    }
  }
  return dialProtocol(node, transport, {
    requireDirect: false,
    streamOpenTimeoutMs: turnDialTimeoutMs,
    diagnostics
  })
}

function shouldRetryBeforeReceipt(error, attempt, receiptConfirmed) {
  if (attempt > 0 || receiptConfirmed) {
    return false
  }
  if (`${error?.a2DispatchStage ?? ''}` !== 'pre-dispatch') {
    return false
  }
  const lower = `${error?.message ?? ''}`.trim().toLowerCase()
  return (
    lower === 'empty json message'
    || lower.includes('turn dial timed out after')
    || lower.includes('peer dial timed out after')
    || lower.includes('protocol stream open timed out after')
    || lower.includes('request receipt timed out after')
    || lower.includes('stream that is closed')
    || lower.includes('stream closed before drain')
    || lower.includes('stream reset')
    || lower.includes('connection reset')
    || lower.includes('connection closed')
    || lower.includes('no existing peer connection is available')
  )
}

function shouldRetryEmptyPostDispatch(error, attempt) {
  if (attempt > 0) {
    return false
  }
  if (`${error?.a2DispatchStage ?? ''}` !== 'post-dispatch') {
    return false
  }
  return describeError(error).trim().toLowerCase() === 'empty json message'
}

function cleanPeerSessionString(value = '') {
  return `${value ?? ''}`.trim()
}

function invalidateTrustedConversation(sessionStore, {
  peerSessionId = '',
  conversationKey = '',
  diagnostics = null,
  reason = ''
} = {}) {
  const removed = sessionStore?.forgetTrustedSession?.({
    peerSessionId,
    conversationKey
  }) ?? null
  emitDiagnostic(diagnostics, 'trusted-session-invalidated', {
    peerSessionId: cleanPeerSessionString(peerSessionId),
    conversationKey: cleanPeerSessionString(conversationKey),
    reason: cleanPeerSessionString(reason),
    removedPeerSessionId: cleanPeerSessionString(removed?.peerSessionId),
    removedRemotePeerId: cleanPeerSessionString(removed?.remotePeerId)
  })
  return removed
}

function describeError(error = null, seen = new Set()) {
  if (error == null) {
    return ''
  }
  if (typeof error !== 'object') {
    return cleanPeerSessionString(error)
  }
  if (seen.has(error)) {
    return ''
  }
  seen.add(error)

  const parts = []
  const message = cleanPeerSessionString(error.message)
  if (message) {
    parts.push(message)
  }
  const code = cleanPeerSessionString(error.code)
  if (code && !parts.some((part) => part.includes(code))) {
    parts.push(code)
  }
  const cause = describeError(error.cause, seen)
  if (cause) {
    parts.push(`cause: ${cause}`)
  }
  if (Array.isArray(error.errors)) {
    for (const nested of error.errors) {
      const detail = describeError(nested, seen)
      if (detail) {
        parts.push(`nested: ${detail}`)
      }
    }
  }
  return [...new Set(parts)].join('; ')
}

function withDeliveryStatusUnknown(error, failureKind = '') {
  const currentMessage = describeError(error)
  const nextMessage = /delivery status is unknown/i.test(currentMessage)
    ? currentMessage
    : `delivery status is unknown after the request was dispatched: ${currentMessage || 'response could not be confirmed'}`
  const wrapped = new Error(nextMessage, { cause: error })
  const originalName = cleanPeerSessionString(error?.name)
  if (originalName) {
    wrapped.name = originalName
  }

  if (error && typeof error === 'object') {
    for (const key of Reflect.ownKeys(error)) {
      if (key === 'message' || key === 'stack' || key === 'cause') {
        continue
      }
      const descriptor = Object.getOwnPropertyDescriptor(error, key)
      if (!descriptor) {
        continue
      }
      try {
        Object.defineProperty(wrapped, key, descriptor)
      } catch {
        // Best effort only.
      }
    }
  }

  wrapped.a2DispatchStage = cleanPeerSessionString(wrapped.a2DispatchStage || error?.a2DispatchStage) || 'post-dispatch'
  wrapped.a2DeliveryStatusKnown = Boolean(error?.a2DeliveryStatusKnown) ? error.a2DeliveryStatusKnown : false
  wrapped.a2FailureKind = cleanPeerSessionString(failureKind) || inferPostDispatchFailureKind(error)
  return wrapped
}

function withAppendedErrorDetail(error, detail = '') {
  const cleanDetail = cleanPeerSessionString(detail)
  if (!cleanDetail) {
    return error
  }
  const currentMessage = cleanPeerSessionString(error?.message)
  const nextMessage = currentMessage.includes(cleanDetail)
    ? currentMessage
    : `${currentMessage || 'delivery status is unknown after the request was dispatched'} ${cleanDetail}`.trim()
  const wrapped = new Error(nextMessage, { cause: error })
  const originalName = cleanPeerSessionString(error?.name)
  if (originalName) {
    wrapped.name = originalName
  }
  if (error && typeof error === 'object') {
    for (const key of Reflect.ownKeys(error)) {
      if (key === 'message' || key === 'stack' || key === 'cause') {
        continue
      }
      const descriptor = Object.getOwnPropertyDescriptor(error, key)
      if (!descriptor) {
        continue
      }
      try {
        Object.defineProperty(wrapped, key, descriptor)
      } catch {
        // Best effort only.
      }
    }
  }
  wrapped.a2DispatchStage = cleanPeerSessionString(wrapped.a2DispatchStage || error?.a2DispatchStage) || 'post-dispatch'
  wrapped.a2DeliveryStatusKnown = Boolean(error?.a2DeliveryStatusKnown) ? error.a2DeliveryStatusKnown : false
  wrapped.a2FailureKind = cleanPeerSessionString(error?.a2FailureKind) || inferPostDispatchFailureKind(error)
  return wrapped
}

function buildJsonRpcError(error = {}) {
  const out = new Error(`${error.message ?? 'remote peer returned an error'}`)
  out.code = Number.parseInt(`${error.code ?? 500}`, 10) || 500
  if (error?.data && typeof error.data === 'object') {
    out.data = error.data
    out.a2RemoteError = error.data
    out.a2FailureKind = cleanPeerSessionString(error.data.failureKind || error.data.kind) || 'remote-runtime-error'
  } else {
    out.a2FailureKind = 'remote-json-rpc-error'
  }
  out.a2DeliveryStatusKnown = true
  return out
}

async function bestEffortAgentCard(apiBase, agentId, bundle, targetAgentId, transport) {
  try {
    return await getAgentCard(apiBase, agentId, bundle, targetAgentId, transport)
  } catch {
    return null
  }
}

function isTrustedSessionRetryable(error) {
  const message = describeError(error)
  const lower = message.toLowerCase()
  const code = Number.parseInt(`${error?.code ?? 0}`, 10) || 0
  if (code === 401 || message.includes('relayConnectTicket or a trusted peerSessionId is required')) {
    return true
  }
  const failureKind = inferPostDispatchFailureKind(error)
  const isPostDispatchTransportFailure = failureKind.startsWith('post-dispatch-') && failureKind !== 'post-dispatch-response-timeout'

  if (`${error?.a2DispatchStage ?? ''}` !== 'pre-dispatch' && !isPostDispatchTransportFailure) {
    return false
  }
  return [
    'target transport is missing dialaddrs',
    'target transport is missing peerid',
    'target transport is missing streamprotocol',
    'no connection was available',
    'no existing peer connection is available',
    'direct p2p upgrade did not complete',
    'connection refused',
    'connection reset',
    'connection closed',
    'stream reset',
    'stream closed before drain',
    'stream that is closed',
    'empty json message',
    'request receipt timed out after',
    'no_reservation',
    'no reservation',
    'reservation',
    'the operation was aborted',
    'already aborted',
    'fetch failed',
    'dial timeout',
    'timed out'
  ].some((pattern) => lower.includes(pattern))
}

function isRelayPresenceRetryable(error) {
  const message = `${error?.message ?? ''}`.trim().toLowerCase()
  return (
    message.startsWith('409 target agent is not currently online') ||
    message.startsWith('409 target agent presence is invalid or stale') ||
    message.startsWith('409 target agent has not published a current peer identity for direct p2p contact') ||
    message.startsWith('409 target agent has not published a current relay reservation or public direct dial address for p2p contact')
  )
}

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

function mergeTargetTransport({
  primary = null,
  secondary = null,
  tertiary = null,
  streamProtocol = ''
} = {}) {
  const sources = [primary, secondary, tertiary].filter((value) => value && typeof value === 'object')
  const peerId = firstNonEmpty(sources.map((value) => value.peerId))
  const protocol = firstNonEmpty(sources.map((value) => value.streamProtocol).concat(streamProtocol))
  const dialAddrs = unique(
    sources.flatMap((value) => value.dialAddrs ?? [])
  )
  const listenAddrs = unique(
    sources.flatMap((value) => value.listenAddrs ?? [])
  )
  const relayAddrs = unique(
    sources.flatMap((value) => value.relayAddrs ?? [])
  )
  const supportedBindings = unique(
    sources.flatMap((value) => value.supportedBindings ?? [])
  )
  const a2aProtocolVersion = firstNonEmpty(sources.map((value) => value.a2aProtocolVersion))

  if (!peerId || !protocol) {
    return null
  }

  return {
    peerId,
    streamProtocol: protocol,
    dialAddrs,
    listenAddrs,
    relayAddrs,
    supportedBindings,
    a2aProtocolVersion
  }
}

function buildInboundRemoteTransport({
  connection,
  remotePeerId,
  binding
} = {}) {
  const remoteAddr = cleanAddr(connection?.remoteAddr?.toString?.())
  const dialAddrs = unique(remoteAddr ? [remoteAddr] : [])
  return {
    peerId: `${remotePeerId ?? ''}`.trim(),
    streamProtocol: `${binding?.streamProtocol ?? ''}`.trim(),
    dialAddrs,
    listenAddrs: dialAddrs
  }
}

function cleanAddr(value) {
  return `${value ?? ''}`.trim()
}

function firstNonEmpty(values = []) {
  for (const value of values) {
    const cleaned = `${value ?? ''}`.trim()
    if (cleaned) {
      return cleaned
    }
  }
  return ''
}

function unique(values = []) {
  return [...new Set(values.map((value) => `${value}`.trim()).filter(Boolean))]
}

function parseConnectTicketId(token) {
  const parts = `${token ?? ''}`.trim().split('.')
  if (parts.length < 2) return ''
  try {
    const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'))
    return `${payload.tid ?? payload.jti ?? ''}`.trim()
  } catch {
    return ''
  }
}
