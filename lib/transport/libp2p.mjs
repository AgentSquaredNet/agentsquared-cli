...
const DEFAULT_DIAL_ADDR_TIMEOUT_MS = 15 * 1000
const DEFAULT_STREAM_OPEN_TIMEOUT_MS = 60 * 1000
...
export function requireListeningTransport(node, binding) {
  ...
  const supportedBindings = unique(
    Array.isArray(binding?.supportedBindings) && binding.supportedBindings.length > 0
...
async function withAbortTimeout(label, timeoutMs, run) {
  const normalizedTimeoutMs = Math.max(250, Number.parseInt(`${timeoutMs ?? 0}`, 10) || DEFAULT_STREAM_OPEN_TIMEOUT_MS)
  const controller = new AbortController()
  const timeout = setTimeout(() => {
    controller.abort(new Error(`${label} timed out after ${normalizedTimeoutMs}ms`))
  }, normalizedTimeoutMs)
  try {
    return await run(controller.signal)
  } catch (error) {
    if (controller.signal.aborted) {
      const timeoutError = new Error(`${label} timed out after ${normalizedTimeoutMs}ms`, { cause: error })
      timeoutError.code = 'A2_P2P_DIAL_TIMEOUT'
      throw timeoutError
    }
    throw error
  } finally {
...
export async function dialProtocol(node, transport, {
  ...
  dialTimeoutMs = DEFAULT_DIAL_ADDR_TIMEOUT_MS,
  ...
  emitDiagnostic(diagnostics, 'dial-start', {
    peerId: transport.peerId,
    dialAddrCount: dialAddrs.length,
    requireDirect,
    dialTimeoutMs,
    streamOpenTimeoutMs
  })
  let lastError = null
  for (let index = 0; index < dialAddrs.length; index += 1) {
    const value = dialAddrs[index]
    const dialStartedAt = Date.now()
    emitDiagnostic(diagnostics, 'dial-address-start', {
      peerId: transport.peerId,
      addrIndex: index,
      addr: value
    })
    try {
      await withAbortTimeout('peer dial', dialTimeoutMs, (signal) => node.dial(multiaddr(value), { signal }))
      emitDiagnostic(diagnostics, 'dial-address-finish', {
        peerId: transport.peerId,
        addrIndex: index,
        addr: value,
      ...
      lastError = error
      emitDiagnostic(diagnostics, 'dial-address-error', {
        peerId: transport.peerId,
        addrIndex: index,
        addr: value,
        durationMs: Date.now() - dialStartedAt,
  ...
  if (lastError && node.getConnections(peerIdFromString(transport.peerId)).length === 0) {
    emitDiagnostic(diagnostics, 'dial-error', {
      peerId: transport.peerId,
      message: `${lastError?.message ?? ''}`.trim(),
      code: `${lastError?.code ?? ''}`.trim()
    })
    throw lastError
  }
  ...
  emitDiagnostic(diagnostics, 'stream-open-start', {
    peerId: transport.peerId,
    viaExistingConnection: false,
    remoteAddr: connection?.remoteAddr?.toString?.() ?? '',
    connectionDirect: isDirectConnection(connection)
  })
  try {
    const stream = await withAbortTimeout('protocol stream open', streamOpenTimeoutMs, (signal) => connection.newStream([transport.streamProtocol], {
      ...(newStreamOptions(connection) ?? {}),
      signal
    }))
    emitDiagnostic(diagnostics, 'stream-open-finish', {
      peerId: transport.peerId,
      viaExistingConnection: false,
    ...
    emitDiagnostic(diagnostics, 'stream-open-error', {
      peerId: transport.peerId,
      viaExistingConnection: false,
      durationMs: Date.now() - streamStartedAt,
      message: `${error?.message ?? ''}`.trim(),
