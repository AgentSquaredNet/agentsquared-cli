...
import { stripAgentSquaredPrefix } from './agent_id.mjs'
...
export function safeAgentId(value) {
  return stripAgentSquaredPrefix(value).replace(/[^a-zA-Z0-9_.-]+/g, '_').toLowerCase()
}

function resolveExistingChildCaseInsensitive(parentDir, childName) {
  const resolvedParent = clean(parentDir) ? resolveUserPath(parentDir) : ''
  const resolvedChild = clean(childName)
  if (!resolvedParent || !resolvedChild || !fs.existsSync(resolvedParent)) {
    return ''
  }
  const exact = path.join(resolvedParent, resolvedChild)
  if (fs.existsSync(exact)) {
    return exact
  }
  const target = resolvedChild.toLowerCase()
  const matches = fs.readdirSync(resolvedParent, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && clean(entry.name).toLowerCase() === target)
    .map((entry) => path.join(resolvedParent, entry.name))
    .sort((left, right) => left.localeCompare(right))
  return matches[0] || ''
}

function resolveAgentSquaredRootUnder(workspaceDir = '') {
  const resolvedWorkspace = clean(workspaceDir) ? resolveUserPath(workspaceDir) : ''
  if (!resolvedWorkspace) {
    return ''
  }
  return resolveExistingChildCaseInsensitive(resolvedWorkspace, 'AgentSquared')
    || path.join(resolvedWorkspace, 'AgentSquared')
}
...
export function resolveAgentSquaredDir(args = {}, detectedHostRuntime = null) {
  ...
  if (workspaceDir) {
    return resolveAgentSquaredRootUnder(workspaceDir)
  }
  if (process.env.HOME) {
    const runtime = clean(detectedHostRuntime?.resolved || detectedHostRuntime?.id)
    if (runtime === 'hermes' && clean(detectedHostRuntime?.hermesHome)) {
      return resolveAgentSquaredRootUnder(detectedHostRuntime.hermesHome)
    }
    return resolveAgentSquaredRootUnder(path.join(process.env.HOME, '.openclaw', 'workspace'))
  }
  return resolveAgentSquaredRootUnder(process.cwd())
}
...
export function resolveAgentScopeDir(agentNameOrId, args = {}, detectedHostRuntime = null) {
  ...
  const agentsquaredDir = resolveAgentSquaredDir(args, detectedHostRuntime)
  return resolveExistingChildCaseInsensitive(agentsquaredDir, safeId)
    || path.join(agentsquaredDir, safeId)
}
...
export function inferAgentSquaredScopeFromArtifact(filePath) {
  ...
  if ((name === 'inbox.sqlite' || name === 'inbox.md') && path.basename(parent) === 'inbox') {
    return grandparent
  }
