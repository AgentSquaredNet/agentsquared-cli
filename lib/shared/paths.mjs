...
export function resolveAgentSquaredDir(args = {}, detectedHostRuntime = null) {
  ...
  if (process.env.HOME) {
    return path.join(process.env.HOME, '.a2')
  }
  return path.join(process.cwd(), '.a2')
}

export function resolveAgentsRootDir(args = {}) {
  return path.join(resolveAgentSquaredDir(args), 'agents')
}
...
export function resolveAgentScopeDir(agentNameOrId, args = {}, detectedHostRuntime = null) {
  ...
  const agentsRootDir = resolveAgentsRootDir(args)
  return resolveExistingChildCaseInsensitive(agentsRootDir, safeId)
    || path.join(agentsRootDir, safeId)
}
